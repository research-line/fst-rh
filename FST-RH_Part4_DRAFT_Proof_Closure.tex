...
Within the even sector, we verify $\mathrm{gap}(\lambda) = \lambda_2^+ - \lambda_1^+ > 0$ via Cauchy interlacing with $N$-convergence. Server computations (16 values of $\lambda \in [5,200]$, $N$ up to 120) confirm:

\medskip
\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{r|c|c|c|c|c}
$\lambda$ & $N_{\mathrm{final}}$ & $\mathrm{gap}^+$ & Cauchy bound & $|v_0(n^*)|^2$ & $n^*$ \\\hline
5 & 60 & $2.69\times 10^{-1}$ & $2.54\times 10^{-1}$ & 0.55 & 8 \\
10 & 60 & $1.59\times 10^{-1}$ & $1.59\times 10^{-1}$ & 0.28 & 31 \\
20 & 100 & $5.44\times 10^{-1}$ & $4.95\times 10^{-1}$ & 0.18 & 27 \\
50 & 80 & $7.42\times 10^{0}$ & $6.45\times 10^{0}$ & 0.50 & 2 \\
100 & 80 & $2.19\times 10^{0}$ & $2.19\times 10^{0}$ & 0.44 & 2 \\
200 & 80 & $3.85\times 10^{0}$ & $3.85\times 10^{0}$ & 0.40 & 2 \\
\end{tabular}
\end{center}

\medskip\noindent
\textbf{Observation:} All 16 tested values have strictly positive Cauchy bound (minimum $0.155$ at $\lambda=25$), confirming that $\lambda_1^+$ is simple within the even sector. Two regimes emerge: for $\lambda \leq 30$, the ground state localizes at high-frequency modes $n^* \approx 2L\gamma_k/\pi$ resonating with zeta zeros $\gamma_k$; for $\lambda \geq 40$, the low-frequency mode $n^*=2$ dominates.
...
5 & 100 & $-6.61$ & $-6.63$ & ODD & \textbf{---} \\
10 & 100 & $-8.91$ & $-8.89$ & EVEN$^*$ & \checkmark$^*$ \\
20 & 100 & $-9.94$ & $-10.4$ & ODD & \textbf{---} \\
25 & 100 & $-10.2$ & $-10.4$ & ODD & \textbf{---} \\
28 & 100 & $-11.2$ & $-10.4$ & EVEN & \checkmark \\
30 & 100 & $-11.8$ & $-10.4$ & EVEN & \checkmark \\
50 & 100 & $-15.7$ & $-11.1$ & EVEN & \checkmark \\
100 & 100 & $-18.7$ & $-14.3$ & EVEN & \checkmark \\
200 & 100 & $-34.6$ & $-26.2$ & EVEN & \checkmark \\
\end{tabular}
\end{center}

\medskip\noindent
$^*$\,\textit{Marginal: $|\lambda_1^+ - \lambda_1^-| \approx 0.025$.}

\medskip\noindent
\textbf{Honest assessment:}
\begin{itemize}
  \item For $\lambda \geq 28$: The even sector minimum is clearly lower ($|\lambda_1^+ - \lambda_1^-| \geq 0.75$, growing with $\lambda$). Theorem~6.1 prerequisites are numerically confirmed.
  \item For $\lambda \leq 25$ (excluding $\lambda=10$): The odd sector has the lower minimum. Theorem~6.1 is \emph{not directly applicable} in this regime.
  \item \textbf{Non-monotone transition:} $\lambda = 10$ shows marginal even dominance, $\lambda = 13$--$25$ show odd dominance, and $\lambda \geq 28$ show robust even dominance. This non-monotonicity reflects the interplay between the archimedean kernel and prime-resonance contributions.
  \item All 19 tested values have \textbf{positive Cauchy bounds} in the even sector (minimum $0.21$ at $\lambda = 8$), confirming simplicity of $\lambda_1^+$.
  \item The dominant mode $n^*$ transitions from high-frequency resonances ($n^* \approx 70$--$98$ for $\lambda \leq 25$) to $n^* = 2$ for $\lambda \geq 25$, with eigenvector weight $|v_0(n^*)|^2 \approx 0.45$.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Even dominance for large $\lambda$:} For $\lambda \geq 30$, the even sector has the overall minimum, confirming both prerequisites of Theorem~6.1. The margin grows with $\lambda$.
  \item \textbf{Odd dominance for small $\lambda$:} For $\lambda \leq 25$, the odd sector is consistently lower. This means Theorem~6.1 is not directly applicable in this regime.
\end{enumerate}

Three structural features emerge:

\begin{enumerate}
  \item \textbf{Mode transition at $\lambda \approx 25$:} The dominant eigenvector mode shifts from high-frequency resonances ($n^* \sim 70$--$98$, correlated with zeta zeros $\gamma_k$ via $n^* \approx 2L\gamma_k/\pi$) to the low-frequency mode $n^* = 2$. This transition coincides with the even-dominance crossover.

  \item \textbf{Non-monotone sector ordering:} $\lambda = 10$ shows marginal even dominance ($\Delta \approx 0.025$), followed by a band $\lambda = 13$--$25$ of odd dominance, and robust even dominance for $\lambda \geq 28$. This non-monotonicity reflects the competition between archimedean and prime contributions at intermediate scales.

  \item \textbf{Growing even-sector gap:} Both the Cauchy bound and the sector gap grow with $\lambda$: the Cauchy bound increases from $0.21$ ($\lambda=8$) to $14.3$ ($\lambda=200$), while the sector margin grows from $0.75$ ($\lambda=28$) to $8.3$ ($\lambda=200$).
\end{enumerate}

\textbf{Implication for Connes' program:} Since Connes needs even dominance for sufficiently large $\lambda$ (the limit $\lambda \to \infty$ drives the approximation to the full zeta function), the ODD regime for small $\lambda$ is not an obstruction. For all $\lambda \geq 28$, both conditions (simplicity and even parity) are numerically confirmed, with growing margins.

A formal proof that $\lambda_1^+ < \lambda_1^-$ for all $\lambda \geq \lambda_0$ (with explicit $\lambda_0$) remains an open problem.
...
\begin{itemize}
  ...
  \item $W_{\mathrm{prime}}$: the prime shift operators $S_{m\log p} + S_{-m\log p}$ are the \emph{sole driver} of even-dominance. At $\lambda = 100$: $\lambda_1^+(W_{\mathrm{prime}}) \approx -27.1$ vs.\ $\lambda_1^-(W_{\mathrm{prime}}) \approx -22.3$.
\end{itemize}
...
\]
For the sine (odd) basis, the second term enters with a minus sign. The difference matrix $D = W_{\mathrm{prime}}^+ - W_{\mathrm{prime}}^-$ is indefinite (trace $\approx +40$, minimum eigenvalue $\approx -20$ at $\lambda = 100$), and the negative eigenspace of $D$ aligns with the ground state direction, producing the even-sector advantage.
...
Removing the constant mode ($n=0$) from the cosine sector changes $\Delta = \lambda_1^+ - \lambda_1^-$ by only $0.03$--$0.3$ out of a total gap of $2.5$--$8.5$. Even dominance is thus a \emph{collective effect} of all cosine modes, not a consequence of the DC component. This distinguishes the Weil operator from the standard prolate operator, where the constant mode plays a more prominent role.
\end{remark}
...
\begin{center}
\begin{tabular}{ccccc}
$\lambda$ & $\|W^+\|_{\mathrm{off}}$ & $\|W^-\|_{\mathrm{off}}$ & ratio & source \\
\hline
30  & 41.2 & 24.0 & 1.72 & prime ($2.03\times$) + arch ($1.08\times$) \\
100 & 57.5 & 32.0 & 1.80 & prime ($1.93\times$) + arch ($1.06\times$) \\
200 & 80.5 & 43.8 & 1.84 & prime ($1.80\times$) + arch ($1.05\times$)
\end{tabular}
\end{center}

The even-sector off-diagonal norm is consistently $\sim 1.8\times$ the odd-sector norm, driven almost entirely by the prime contributions (factor $\sim 2$). This wider eigenvalue spread pushes the minimum eigenvalue further below the mean.

For small $\lambda$, the Schur--Horn bound is tight enough to prove even dominance rigorously: at $\lambda = 30$, the bound gives $\lambda_1^+ \leq -6.71 < -5.49 = \lambda_1^-$, and at $\lambda = 50$, $\lambda_1^+ \leq -7.17 < -6.73 = \lambda_1^-$. For larger $\lambda$, the bound is not sufficiently sharp (since the actual eigenvalues deviate by $1$--$3$ standard deviations from the mean, while the bound uses only the first standard deviation). A tighter approach uses the Rayleigh--Ritz variational principle: the ground state concentrates on the first $k = 3$--$5$ cosine modes, and the $k \times k$ principal submatrix already suffices. Specifically, by Cauchy interlacing,
\[ \lambda_1(\mathrm{QW}_\lambda^+) \leq \lambda_1(\mathrm{QW}_\lambda^+[{:}k,{:}k]), \]
and we verify numerically that $\lambda_1(\mathrm{QW}_\lambda^+[{:}k,{:}k]) < \lambda_1(\mathrm{QW}_\lambda^-)$ for all tested $\lambda$:
...
Moreover, $\lambda_1(\mathrm{QW}_\lambda^-[{:}N])$ remains above $\lambda_1(\mathrm{QW}_\lambda^+[{:}4])$ for \emph{all} $N \leq 50$ (verified at $\lambda = 100, 200$), indicating that the gap is genuine and not an artifact of truncation. A computer-assisted proof using interval arithmetic on the $4 \times 4$ matrix elements (which are explicit sums of trigonometric integrals over primes) appears feasible and would complete the formal verification for each fixed $\lambda$.
