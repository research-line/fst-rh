...
Hier entwickeln wir den Spektralansatz vollst"andig. Gem"a\ss{} Connes' "Ubersicht von 2026 \cite{Connes2025} untersuchen wir die quadratische Weil-Form $\mathrm{QW}_\lambda$ und ihre Gerade-Ungerade-Zerlegung. Unsere drei Hauptbeitr"age sind:
\begin{enumerate}
...
Eine aktuelle "Ubersicht von Connes \cite{Connes2025} reduziert die Riemannsche Vermutung auf eine spektrale Bedingung der quadratischen Weil-Form $\mathrm{QW}_\lambda$. Wir fassen den Zusammenhang zusammen und pr"asentieren numerische Belege f"ur den verbleibenden offenen Schritt.
...
\end{multline*}
wobei $T_u$ den Shift-Operator bezeichnet, $\gamma$ die Euler-Mascheroni-Konstante ist und der archimedische Kern $e^{u/2}/(2\sinh u)$ aus dem Variablenwechsel $x = e^u$ resultiert, angewendet auf Connes' Gleichung~(10) in \cite{Connes2025}, mit $x^{1/2}/(x-x^{-1})\,d^*\!x = e^{u/2}/(2\sinh u)\,du$. Der Operator $A_\lambda$ ist nach unten beschr"ankt mit kompaktem Resolventen (daher diskretes Spektrum).
...
Unsere numerischen Ergebnisse adressieren das offene Problem von Connes (Abschnitt~6.6 von \cite{Connes2025}) von zwei Seiten:
\begin{enumerate}
...
\subsubsection{Lemma L3: Niedrig-hoch-Kopplung ist nur polynomiell}

\begin{lemma}[Off-diagonale Kontrolle]
\label{lem:L3}
Für $B = \mathrm{QW}[{:}K,\,K{:}]$ existiert ein $\gamma > 0$ so dass $\|B\|_{\mathrm{op}} \leq \gamma L$ für alle $\lambda \geq \lambda_0$.
\end{lemma}
\begin{proof}[Beweis][Beweis via Schur-Test]
$\sup_i \sum_j |B_{ij}| \leq c_1 L$ und $\sup_j \sum_i |B_{ij}| \leq c_2 L$ (jede Zeilen-/Spaltensumme beinhaltet $O(L/\log 2)$ Primzahl-Verschiebungsterme, jeder $O(1)$). Der Schur-Test ergibt $\|B\|_{\mathrm{op}} \leq \sqrt{c_1 c_2}\,L$.
\end{proof}

\noindent Numerisch: $\|B\|_{\mathrm{op}}/L \in [0.23, 1.64]$ für $\lambda \in [30, 500]$.
...
\emph{Schritt~3 (Lückenunterschranke mit Paritätssplit).} Das Auto-Überlappungsprofil ist $h_n(u) = \tfrac{1}{2}(1-u/2)\cos(n\pi u) - \sin(n\pi u)/(4n\pi)$. Am Laplace-Endpunkt $u = 1$: $h_n(1) = (-1)^n/4$. Die primgewichtete Diagonale wird dominiert durch diesen Endpunkt mittels des Laplace-Prinzips:
\[
...
\emph{Schritt~4 (Paritätsgepaarte Kopplungsunterdrückung).} Für ungerade $j \geq 3$ verschwindet die \emph{Kreuz}-Überlappung $S^+(1,j;\delta,L)$ auch am Laplace-Endpunkt: $S^+(1,j;L,L) = (-1)^j\delta_{1j}/2 = 0$ für $j \neq 1$ (durch Orthogonalität auf $[0,L]$). Die Kopplung $B_{1j}$ stammt daher aus dem Subleading-Laplace-Term, ergebend $|B_{1j}| = O(j\sqrt{\lambda}/L)$ anstatt $O(\sqrt{\lambda})$. Das Verhältnis:
\[
...
\begin{proof}[Beweis]
Nach \Cref{prop:euler-maclaurin}, $\rho^{\mathrm{EM}}(L) < 0$ für $L \geq 13$ (d.h., $\lambda \geq 442{,}413$), zertifiziert durch Intervallarithmetik. Nach \Cref{lem:pnt-transfer}, $|\rho(\lambda) - \rho^{\mathrm{EM}}(\lambda)| = O(L\,e^{-c\sqrt{L}}) \to 0$. Wähle $L_1 \geq 13$ sodass $|\rho(\lambda) - \rho^{\mathrm{EM}}(\lambda)| < |\rho^{\mathrm{EM}}(L_1)|/2$ für alle $\lambda \geq e^{L_1}$. Dann für $\lambda_0 \coloneqq e^{L_1}$:
\[
  \rho(\lambda) < \rho^{\mathrm{EM}}(L_1)/2 < 0 \quad\text{for all } \lambda \geq \lambda_0.
\]
Da $\rho < 0$ bedeutet $E_{\sin} < E_{\cos}$, haben wir $E_{\sin} - E_{\cos} < 0 < (1-\eta)\,D$ für jedes $\eta \in (0,1)$.
\end{proof}
...
Zwischen benachbarten Zertifikatswerten kann die zertifizierte Lücke nicht die Null durchqueren durch ein strukturelles Argument: (i)~nach dem Shift-Parity-Lemma (\Cref{thm:shift-parity}) erfüllt jede neue Primzahl $p$, die die Summe bei $\lambda = p$ betritt, $\lambda_1(W_p^+) < \lambda_1(W_p^-)$, daher \emph{vertiefen} neue Primzahlen die gerade-ungerade-Lücke; (ii)~nach der Hellmann--Feynman-Analyse, $d(\mathrm{gap})/dL > 0$ für $\lambda \geq 80$, daher vertieft auch die $L$-Variation die Lücke; (iii)~die intrageradege spektrale Lücke ($\lambda_2^+ - \lambda_1^+ \geq 8.69$ bei $\lambda = 100$, \Cref{lem:simplicity}) übersteigt die Operatornorm jeder einzelnen Primzahl-Störung ($\|W_p\|_{\mathrm{op}} < 0.46$ für $p \geq 100$) um einen Faktor $\geq 18$, was Eigenvektorstabilität unter jeder diskreten Primzahl-Addition sichert.
