...
The certifier program (\texttt{certifier\_production.py}) implements the certified-gap computation for a geometric grid of $\lambda$-values. In the present v2.3 status this output is treated as a finite certificate \emph{conditional on} the odd-sector tail lower bound described below; the even-sector upper bound is interval-arithmetic rigorous, while the odd-sector lower-bound mechanism still requires an independent tail-bound validation in the Connes--van Suijlekom operator setting:
\begin{itemize}
