...
\]
for $\lambda \geq e^{13} = 442{,}413$, where $C < 2$ is an effective constant absorbing the ratio structure of the resolvent energies (each $|B_j|^2/g_j$ error is bounded by $\Delta B \cdot (2|B| + \Delta B)/g$, with $\Delta B/B < 1/(20L)$ from the Dusart bound); $C$ is numerically observed to lie in $[1.0, 1.6]$ on the tested range but has not been derived as a rigorous closed-form constant (see \Cref{rem:C-constant-caveat}). Since $0.005 < 0.0109/2$, we obtain:
\[
...
\begin{remark}[Status of the constant $C$ in the PNT transfer]
\label{rem:C-constant-caveat}
The bound $C < 2$ used above is derived heuristically from the product rule applied to the relative perturbation $\Delta B/B$ on each resolvent-energy ratio; a fully rigorous derivation would require tracking the worst-case cross-term between primes-contribution errors and gap-denominator errors uniformly in $j$. Numerical evaluation on $\lambda \in [500, 10^6]$ gives effective values $C_{\mathrm{eff}} \in [1.0, 1.6]$, comfortably below the invoked bound. A rigorous closed-form bound $C \leq C^\ast$ with an explicit admissible $C^\ast$ remains to be established; the Regime~2 threshold is therefore \emph{conditional on} $C < 2$. The safety margin $0.005$ allows $C$ up to roughly $2.2$ before the margin is exhausted.
\end{remark}
