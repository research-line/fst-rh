...
This is the second paper in a five-part series (Part~I: Foundations and Obstructions \cite{Geiger2026partI}; Part~III: Spectral Routes \cite{Geiger2026partIII}; Part~IV: Cross-Route Synthesis \cite{Geiger2026partIV}; Part~V: Conclusio \cite{Geiger2026partV}). We address the open problem identified in Connes' 2026 survey \cite{Connes2026}: whether the minimum eigenvalue of the Weil quadratic form~$\mathrm{QW}_\lambda$ is simple with even eigenfunction. We separate three outputs at different proof levels. First, the \emph{Shift Parity Lemma}: the difference matrix between even and odd shift operators has strictly negative minimum eigenvalue for all $N \geq 3$ modes and all normalized shifts $r \in (0,2)$, implying that every prime individually favors even eigenfunctions. Second, we report computer-assisted finite-range gap diagnostics using interval arithmetic for $33$ values of $\lambda$ from $100$ to $1{,}300{,}000$; these are a strong conditional bridge until the odd-sector lower bound is independently validated. Third, we prove the \emph{Leading-Mode Cancellation Lemma}: the overlap differences between even and odd sectors cancel pairwise with exact constant $c = 2 + \sqrt{2}$, yielding $(E_{\sin} - E_{\cos})/D(\lambda) = O(1/\log\lambda) \to 0$. In v2.0 we add two non-existence theorems (Theorems~\ref{thm:NE-A} and~\ref{thm:NE-B}) that formally rule out the absolute total-positivity and universal-commuting-operator routes, establishing that even dominance is necessarily an arithmetic-statistical rather than a purely algebraic phenomenon. In v2.1 we add a robust Direct Frontier-Dominance argument (\Cref{sec:direct-frontier}, \Cref{prop:direct-frontier}) based on a common Rayleigh test vector on the frontier window $r \in [0.7,1]$.

\medskip\noindent
\textbf{Status correction (v2.2, 2026-05-14).}
External critical review of the v2.1 manuscript and the companion proof-only extract \cite{Geiger2026evendom} identified a structural gap in the asymptotic argument that was hidden by the previous wording. The Direct Frontier-Dominance proposition establishes
$\lambda_{\min}(W^+_{\mathrm{prime}}-W^-_{\mathrm{prime}})=-\Theta(\sqrt\lambda)$
via Rayleigh evaluation at a common test vector. This is a \emph{variational} statement: there exists a vector $v$ with $v^T W^+ v < v^T W^- v$. It does \emph{not} establish $\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$ asymptotically, because $\lambda_1^-(\lambda)\le v^T W^- v$ runs in the same direction; a separate quantitative lower bound for $\lambda_1^-$ is required. The simple $2\times 2$ counter-example $A=\mathrm{diag}(0,10)$, $B=\mathrm{diag}(5,-5)$ illustrates the gap.
The status of the A1--A8 reduction chain is therefore revised from ``unconditional'' (v2.1) back to \textbf{conditional reduction}: the finite range $100\le\lambda\le 1{,}300{,}000$ provides strong interval-arithmetic diagnostics, but theorem-level even dominance on that range still depends on a validated lower bound for $\lambda_1^-(\lambda)$. Asymptotic even dominance is reduced to the \emph{Asymptotic Variational Gap Conjecture} (a quantitative lower bound for $\lambda_1^-(\lambda)$). A proposed approach via degenerate perturbation theory on the asymmetric three-mode diagonal structure is sketched in the companion paper~\cite{Geiger2026evendom}. The non-existence theorems NE-A and NE-B, the Shift Parity Lemma, the Leading-Mode Cancellation Lemma, and the 33 finite-range diagnostics remain unaffected by this revision.

\medskip\noindent
\textbf{Structural update (v3.0, 2026-05-26).}
The series is restructured from a trilogy to a five-part series; the conditional
reduction status from v2.2 is unchanged. A new \Cref{sec:gf1} presents the
GF1 boundary-moment benchmark
$\mathrm{sLI} \sim (64\pi^2/3)\,\lambda/(NL)$ for the converged
$\lambda=5,7$ benchmark, records the $\lambda=3$ low-bandwidth
outlier separately, and leaves $\lambda=11$ as an open discriminator.

\medskip\noindent
\textbf{Strict-review update (v3.1, 2026-06-29).}
The N1 shifted-inverse finite-section work is not used here as a ground-state
certificate: for $B_\lambda=(A_\lambda+MI)^{-1}$ the spectral map reverses
order, so the $A_\lambda$ ground state corresponds to the largest isolated
$B_\lambda$ eigenvalue, not the smallest. The finite CAP and Direct Frontier
diagnostics below are retained as conditional legacy/variational evidence until
they are replayed in the corrected Connes--van Suijlekom $U_n$ framework with a
validated odd-sector lower bound.
\end{abstract}

{\footnotesize
\noindent\textbf{MSC 2020:} 11M26, 11M36, 47A75, 65G20\\
\textbf{Keywords:} Riemann Hypothesis, Weil quadratic form, even dominance, Shift Parity Lemma, computer-assisted diagnostics
}
...
This paper is the second in a five-part series investigating the Riemann Hypothesis through gauge theory, spectral analysis, and arithmetic thermodynamics. Part~I \cite{Geiger2026partI} establishes the thermodynamic landscape (structural results R1--R9) and documents the failure of the gauge-theoretic approach (obstructions K1--K4), leading to a reorientation toward Connes' spectral program. Part~III \cite{Geiger2026partIII} surveys the spectral routes; Part~IV \cite{Geiger2026partIV} collects cross-route synthesis; Part~V \cite{Geiger2026partV} provides the conclusio.
...
\begin{enumerate}
  ...
  \item \textbf{Computer-assisted finite-range diagnostics} (\Cref{sec:certificates}): strong negative gap diagnostics at $33$ values of $\lambda$ from $100$ to $1{,}300{,}000$ via interval arithmetic; theorem-level even dominance remains conditional on the validated odd-sector lower bound.
...
\begin{itemize}
  ...
  \item \textbf{Computer-assisted diagnostics (interval arithmetic):} A negative finite-range gap diagnostic has been documented for $33$ values of $\lambda$ from $100$ to $1{,}300{,}000$ using interval arithmetic (mpmath.iv, 50~digits) for the even block and float64 tail extrapolation for the odd block. The gap grows monotonically as $\sim\!\sqrt{\lambda}$, from $-2.25$ at $\lambda = 100$ to $-475.83$ at $\lambda = 1{,}300{,}000$; theorem-level certification is conditional on the odd-sector lower-bound validation (see \Cref{sec:certificates}).
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Even dominance for large $\lambda$:} For $\lambda \geq 25$, the finite-dimensional diagnostics put the even sector lower in most tested ranges. The theorem-level infinite-dimensional ordering still needs the odd-sector lower bound.
...
\begin{remark}[Full-space verification, v2.0]
An independent verification in the full Galerkin space ($N = 120$ PSWF modes, primes up to $P_{\max} = 10{,}000$, exact kernel) was performed on a cloud-compute server. At both $\lambda = 100$ and $\lambda = 200$, all $1229/1229$ primes deepen the even--odd gap ($\Delta_p < 0$), with zero exceptions across 2458 individual tests. The small $p = 2$ anomaly observed in the $3 \times 3$ truncation vanishes when enough modes are included. Scripts and CSV results: \texttt{scripts/gemini\_verification/}.
\end{remark}
...
\begin{enumerate}[label=(\Alph*)]
  \item \textbf{Complete monotonicity and total positivity (ruled out, v1.4):} The archimedean kernel admits the Laplace mixture representation
...
The cumulative step is reduced in \Cref{prop:A6} via the \textbf{M1$''$ resolvent framework} combined with PNT transfer and computer-assisted diagnostics (a three-regime argument). This path is closest in spirit to route~(C) (prolate perturbation) but operates directly on the Galerkin representation rather than requiring a separate prolate comparison operator. Routes~(C) and~(D) remain available as independent verification. Route~(A) is excluded as an absolute structural property: the Fourier multiplier $M(\xi) = -2\,\Re[\zeta'/\zeta(\tfrac{1}{2}+i\xi)]$ attains negative values on a set of positive measure (\Cref{thm:NE-A}). Route~(B) is excluded at all tested Galerkin truncations $N \leq 15$ (\Cref{thm:NE-B}); its full-space extension is formulated as \Cref{conj:NE-B-full} and remains open.
\end{remark}

\begin{remark}[Structural interpretation of the excluded routes]
The exclusion of (A) (as an absolute property, \Cref{thm:NE-A}) and (B) (for $N \leq 15$, \Cref{thm:NE-B}) is not a gap in the proof but a structural discovery. Route~(A) would have required the primes to be ``absolutely smoothing'' (variation-diminishing as a single operator-theoretic property); instead, their collective effect is variation-diminishing only after prime-weighted averaging, which is the content of Route~(E). Route~(B) would have required the primes to share a hidden common symmetry that a single differential operator captures; instead, the dense-grid SVD evidence (up to $N=15$) indicates that they are multiplicatively independent and carry no such common structure in the tested truncations. Even dominance nonetheless holds---driven by the pointwise Shift Parity Lemma and the prime-weighted summation, not by an overarching algebraic principle.
\end{remark}

\subsection{Formal non-existence theorems (v2.0)}
...
\begin{theorem}[Non-$\mathrm{PF}_\infty$ of the Prime Shift Operator]\label{thm:NE-A}
Let $L > 0$ and consider the prime shift operator on $L^2([-L,L])$, whose Fourier multiplier is
\[
  M(\xi) \;=\; -2\,\Re\!\left[\frac{\zeta'}{\zeta}\!\left(\tfrac{1}{2}+i\xi\right)\right],
\]
given by the Weil explicit formula (see \cite{Connes2026}), valid unconditionally. The set
\[
  \{\xi \in [-L,L] : M(\xi) < 0\}
\]
has strictly positive Lebesgue measure. Consequently, the operator family $A_\lambda$ does not satisfy absolute total positivity in the sense of $\mathrm{PF}_\infty$: the route to even dominance via absolute total positivity (Schoenberg--Hirschman, Gantmacher--Krein oscillation theory) is thereby excluded.
\end{theorem}

\begin{proof}[Proof sketch]
On any compact interval $[-L,L]$ the nontrivial zeros $\rho_k = \tfrac{1}{2}+i\gamma_k$ of $\zeta$ contribute finitely many simple poles of $\zeta'/\zeta$. Excising $\varepsilon$-neighbourhoods $U_\varepsilon = \bigcup_k (\gamma_k - \varepsilon, \gamma_k + \varepsilon)$ around these ordinates yields a set on which $M(\xi) = -2\,\Re[\zeta'/\zeta(\tfrac{1}{2}+i\xi)]$ is a bounded continuous function (a classical consequence of the Weil explicit formula away from zero ordinates). Near each ordinate, the real part of $1/(s-\rho_k)$ changes sign as $\xi$ crosses $\gamma_k$; hence on any sufficiently small neighbourhood of $\gamma_k$ (excluding the pole itself), the continuous representative of $M$ attains both strictly positive and strictly negative values on open subintervals. The sub-intervals on which $M < 0$ thereby form a non-empty open set, so $\{\xi \in [-L,L]\setminus U_\varepsilon : M(\xi) < 0\}$ has strictly positive Lebesgue measure for every $\varepsilon > 0$ sufficiently small. By monotone convergence ($\varepsilon \to 0^+$), the same holds for $\{\xi \in [-L,L] : M(\xi) < 0\}$ interpreted as a measurable set in the complement of the discrete pole set. Absolute total positivity ($\mathrm{PF}_\infty$) requires the multiplier to be nonnegative almost everywhere in the sense of distributions on the complement of the pole set; a set of positive measure with $M(\xi) < 0$ contradicts this. The measure-theoretic statement is therefore well-defined \emph{modulo the discrete pole set} (which has Lebesgue measure zero), and no distributional regularization is needed for the positivity-of-measure conclusion; see \Cref{rem:NE-A-distribution}.
\end{proof}

\begin{remark}[Distributional status of $M$ and identification with a multiplier]
\label{rem:NE-A-distribution}
The statement of \Cref{thm:NE-A} concerns the measurable function $\xi \mapsto M(\xi)$ on $[-L,L] \setminus \{\gamma_k\}$, which is locally bounded on that set. Identification of $M$ as the Fourier multiplier of the prime shift operator requires interpreting the Weil explicit formula in its tempered-distribution form, with the pole contributions regularized as Cauchy principal values; this is standard (cf.~\cite{Connes2026}, \S3) and does not affect the positivity-of-measure conclusion, since the pole set has Lebesgue measure zero. The argument rules out $\mathrm{PF}_\infty$ as an absolute property of the a.e.-representative of $M$; stronger notions of total positivity (e.g.\ $\mathrm{PF}_k$ for finite $k$) are not discussed here.
\end{remark}


\begin{remark}[Numerical quantification]
Sampling $M(\xi)$ on a uniform grid of $10^4$ points in $\xi \in [0, 100]$ (step $\Delta\xi = 0.01$, $50$-digit mpmath precision for $\zeta'/\zeta$), excluding $\varepsilon$-neighbourhoods of the first $29$ nontrivial ordinates $\gamma_k \in [0,100]$ with $\varepsilon = 10^{-3}$, yields approximately $49\%$ of sampled points satisfying $M(\xi) < 0$. The estimate is stable under refinement ($\varepsilon = 10^{-4}$, $10^5$ points: $49.2\%$). This illustrates the measure-theoretic statement of Theorem~\ref{thm:NE-A} empirically and does not replace the structural argument given above.
\end{remark}

\begin{theorem}[No Universal Commuting Operator, for $N \leq 15$]\label{thm:NE-B}
Let $D_N(r)$ denote the shift-parity difference matrix from \Cref{def:D-matrix}. For each $N \in \{3, 5, 7, 10, 15\}$ and any dense sample $r_1, \ldots, r_{13} \in (0,2)$,
\[
  \ker\bigl\{T \in \mathrm{Sym}_N(\mathbb{R}) \;:\; [T, D_N(r_i)] = 0 \text{ for all } i = 1, \ldots, 13\bigr\} \;=\; \mathrm{span}\{I\}.
\]
...
\begin{conjecture}[Full-space extension]\label{conj:NE-B-full}
The conclusion of Theorem~\ref{thm:NE-B} holds for all $N \in \mathbb{N}$: for any sufficiently dense sample of $r \in (0,2)$, the only symmetric matrices commuting with all corresponding $D_N(r)$ are scalar multiples of the identity. Consequently, no non-trivial universal commuting operator exists in the infinite-dimensional limit.
\end{conjecture}
...
\subsubsection{Computer-Assisted Gap Diagnostics via Interval Arithmetic}
...
The preceding sections establish strong finite-range gap diagnostics for finitely many $\lambda$ ($33$ values up to $\lambda = 1{,}300{,}000$; numerically for all $\lambda \geq 55$). To close the conditional reduction for \emph{all} large $\lambda$, we outline a functional-analytic strategy based on weighted compactness that could avoid the cumulative algebraic step entirely.
...
\label{conj:pw}
There exists $\alpha > \tfrac{1}{2}$ such that
\[
  \sup_\lambda \int |\phi_\lambda(t)|^2\,e^{2\alpha|t|}\,dt < \infty.
\]
Consequently, $\{F_\lambda\}$ converges locally uniformly on the strip
...
% =============================================================================
\section{Galerkin Imbalance: Boundary-Moment Benchmark (v3.0)}
\label{sec:gf1}
% =============================================================================

The finite-dimensional Galerkin approximation to the Weil quadratic form~$\mathrm{QW}_\lambda$ introduces an imbalance between the even and odd sectors that depends on both the cutoff parameter~$\lambda$ and the Galerkin dimension~$N$. We define the \emph{scaled ladder imbalance}
\begin{equation}
\label{eq:sLI}
\mathrm{sLI}(\lambda, N) \;=\; \frac{I_2^{\mathrm{ladder}}(\lambda,N)}{I_1^{\mathrm{ladder}}(\lambda,N)}\,\cdot\,\Gamma(\lambda)^{-1/4},
\end{equation}
where $I_1^{\mathrm{ladder}}$ and $I_2^{\mathrm{ladder}}$ denote the $L^1$-weighted Chebyshev moment ladder sums on the even and odd far-tail sectors, respectively, and $\Gamma(\lambda)$ is the Galerkin quarter-power normalization.

\subsection{Identification of the Boundary-Moment Constant}

Numerical computation on the Mac~Studio (converged benchmark regime
$\xi=NL/\lambda \gtrsim 50$, $\mathrm{dps} = 4N$ for precision
safety) reveals that, for the tested benchmark values
$\lambda\in\{5,7\}$, $\mathrm{sLI}$ is well described by the
boundary-moment benchmark
\begin{equation}
...
\begin{remark}[Derivation candidate]
\label{rem:cstar-derivation}
The value $C^*$ admits the formal integral representation
\[
C^* = 16 \int_0^\infty \frac{t^2}{\sinh^2(t/2)}\,dt = 16\,\cdot\,\frac{4\pi^2}{3} = \frac{64\pi^2}{3},
\]
where $\int_0^\infty t^2/\sinh^2(t/2)\,dt = 4\pi^2/3$ is a standard Bernoulli integral. This is an identification candidate, not a forward derivation. The 27~May audit falsified the naive endpoint factorisation
$K(p,\gamma)=\mathrm{const}\cdot t^2/\sinh^2(t/2)$: in the variable
$t=\log(\gamma/p)$ the exact paired Cauchy kernel is
\[
  K(t)=\frac{e^t\cosh t}{2\sinh^2 t},
\]
with a collar singularity at $t=0$. A proof of $C^*$ therefore requires
the missing eigenmode asymptotics for the weights $q_a(k)$, not only the
scalar kernel identity.
\end{remark}

\begin{table}[h]
\centering
\caption{Data check for the boundary-moment constant $C^*$. The effective constant $C_{\lambda,N} = \mathrm{sLI}\cdot NL/\lambda$ is compared against $C^* = 64\pi^2/3 \approx 210.55$. The $\lambda=5,7$ rows are the current benchmark support; the $\lambda=11$ row is a single open discriminator, not confirmation of a universal $\lambda\geq5$ law. All runs use Chebyshev degree~$80$, $j = 25$.}
\label{tab:gf1-data}
\begin{tabular}{cccccc}
\toprule
$\lambda$ & $N$ & $\xi=NL/\lambda$ & $C_{\lambda,N}$ & deviation & status \\
\midrule
...
\begin{theorem}[GF1: Conditional boundary-moment scaling]
\label{thm:GF1}
Assume that the eigenmode-weighted Galerkin kernel of
$\mathrm{QW}_\lambda$ has a production-sLI limit in which the exact
collar kernel above reduces to the effective boundary moment
$16\int_0^\infty t^2/\sinh^2(t/2)\,dt$. Assume additionally that the
production observable is stable under near-cluster rotations and that the
$\lambda=5,7$ benchmark persists beyond the current $\lambda=11$
discriminator. Then
\[
...
\begin{remark}[Low-bandwidth guardrail]
\label{rem:gf1-lambda3}
The same normalisation is \emph{not} a theorem uniformly down to
$\lambda=3$. A dedicated $N$-sweep at $\lambda=3$ indicates a different
low-bandwidth plateau, with the older $C_m$ normalisation approaching
approximately~$7$ rather than the $\lambda=5,7$ benchmark plateau near
$1.5$--$2.0$. A separate $\lambda=11,N=200$ run gives
$C_{\lambda,N}^{\mathrm{eff}}\approx167.6$, about $20\%$ below
$64\pi^2/3$; the pending $N=280$ and $N=360$ runs decide whether this is
pre-convergence or genuine $\lambda$-dependence. Thus GF1 is currently a
conditional benchmark, not a single $\lambda\geq5$ universal theorem.
\end{remark}

\begin{remark}[Convergence regime]
\label{rem:NL-regime}
The relevant scaling variable is
\[
  \xi=\frac{NL}{\lambda},\qquad L=2\log\lambda,
\]
not $N/L$. For $\lambda=5,7$, the data enter the benchmark window once
$\xi\gtrsim50$--$60$; below that window finite-dimensional boundary
effects distort $C_{\lambda,N}$. The $\lambda=11$ discriminator shows that
the threshold may itself be $\lambda$-dependent.
\end{remark}

\subsection{Hypothesis Correction Chain}
...
\begin{enumerate}
  \item \textbf{Iter~15 (2026-05-24):} Initial hypothesis: $C_m \approx 8$ constant, based on $\lambda = 5$, $N = 55$ data.
  \item \textbf{Iter~18 (2026-05-25):} $C_m \approx 8$ falsified by $N$-convergence sweep; $\mathrm{scaledLadderI}$ decreases monotonically with~$N$.
  \item \textbf{Iter~19 (2026-05-25):} Revised hypothesis: plateau at $C_m \approx 1.5$--$2.0$.
  \item \textbf{Iter~20 (2026-05-26):} Multi-$\lambda$ confirmation ($\lambda = 5, 7$): benchmark consistency in $\xi=NL/\lambda$ confirmed within~$5\%$.
  \item \textbf{Iter~27 (2026-05-26, GPT audit):} Universal constant $C^* = 64\pi^2/3$ identified via integral representation; boundary-moment kernel $16t^2/\sinh^2(t/2)$ as derivation candidate.
  \item \textbf{Iter~28 (2026-05-26):} The $\lambda=3$ $N$-sweep breaks the all-$\lambda$ universal reading. The safe formulation at this stage was a $\lambda=5,7$ boundary-moment benchmark plus a conservative low-bandwidth branch at $\lambda=3$.
  \item \textbf{Iter~29 (2026-05-27):} The endpoint-kernel factorisation is falsified by the exact kernel $K(t)=e^t\cosh(t)/(2\sinh^2t)$, and the $\lambda=11,N=200$ point gives $C_{\mathrm{eff}}\approx168$. The safe formulation is now a $\lambda=5,7$ benchmark with $\lambda=11$ pending.
\end{enumerate}
The initial $C_m$-constancy hypothesis was an artifact of the transient
Galerkin regime. In the converged benchmark regime for $\lambda=5,7$,
$\mathrm{sLI}$ follows the $C^*\lambda/(NL)$ law; the $\lambda=3$
counter-sweep and the open $\lambda=11$ discriminator keep GF1 at the
level of a conditional target rather than a universal theorem.
...
This paper no longer claims to rigorously establish even dominance of the Weil
quadratic form $\mathrm{QW}_\lambda$ unconditionally, even on the finite
certified range. The current architecture combines three ingredients: (1)~the
Shift Parity Lemma, showing that each prime individually favors even
eigenfunctions; (2)~33 computer-assisted finite-range diagnostics providing
strong interval-arithmetic evidence pending the odd-sector lower-bound
validation; and (3)~the M1$''$ resolvent framework together with the v2.1 Direct
Frontier-Dominance argument, both of which establish the asymptotic
variational statement $\lambda_{\min}(W^+-W^-)=-\Theta(\sqrt\lambda)$. The
remaining step from this variational statement to
$\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$ for all sufficiently large
$\lambda$ is formulated in v2.2 as the \emph{Asymptotic Variational Gap
Conjecture}.
...
In v1.1, both Lemma~B and Lemma~C have been upgraded to fully analytical
arguments: the parity-split gap (Lemma~B, Steps~3--4) derives
$c_{\mathrm{gap}} \geq 4\pi^2/L$ from Laplace asymptotics, and the
sector-difference control (Lemma~C) replaces the global Neumann condition
$\|R_0 K\| < 1$ by the tail-symmetry bound
$|E_{\sin}^{\mathrm{tail}} - E_{\cos}^{\mathrm{tail}}|/D \leq C/N_0^2$. No
numerical constants remain in the asymptotic variational analysis of
Regime~2. Version~2.1 adds a robust Direct Frontier-Dominance proof
(\Cref{prop:direct-frontier}) that uses a common Rayleigh vector rather than
eigenvalue-additivity and removes the two earlier v2.0 caveats; the v2.2
revision clarifies that this still yields a conditional reduction rather than
an unconditional closure of A6.
...
\]
Exchanging sums and using Bessel's inequality
($\sum_j |\langle e_i, T_\delta e_j\rangle|^2
\leq \|T_\delta e_i\|^2 \leq 1$,
hence $\sum_j |S_{ij}| \leq \sqrt{N}$ by Cauchy--Schwarz)
yields $\sum_{j \geq K} |S_{i,j+K}^+(\delta, L)|
\leq C_0\sqrt{L}$, so
\[
...
The key to closing the finite bridge is that the observed gap grows monotonically with $\lambda$, not because of abstract operator convergence, but because the Galerkin eigenvalues of the finite blocks scale cleanly. The theorem-level implication requires replacing the candidate odd-tail estimate by a validated lower bound.

\begin{definition}[Conditional certified gap]
...
\end{align*}
By Cauchy interlacing, $\ell_{\cos}^{(k)}(\lambda) \geq \lambda_1^+(\lambda)$ and $\ell_{\sin}^{(N_0)}(\lambda) \geq \lambda_1^-(\lambda)$. Therefore the finite odd block alone is \emph{not} a lower bound. If the tail term $T_{\sin}(\lambda)$ is validated so that
\[
...
The even block uses $k = 4$ modes in interval arithmetic (mpmath.iv, 50-digit precision); the Frobenius perturbation radius is $\mathrm{rad}_{\mathrm{frob}} \leq 0.0025$ at all certificate values. The odd block uses $N_0 = 40$--$90$ modes in float64, with a candidate tail correction from modes $N_0+1$ to $N_{\mathrm{big}} = 60$--$90$ and beyond. At $\lambda = 100$ (the worst case), the total odd-sector tail estimate is $0.059$ (\texttt{sin\_info.total\_tail} in the certificate data). The combined reported margin is $\leq 0.0025 + 0.059 = 0.061$, while $|\mathrm{cert\_gap}| = 2.11$---a safety factor of $\mathbf{34\times}$ if the odd-tail estimate is validated. For $\lambda \geq 200$, the safety factor exceeds $65\times$ and grows monotonically. This makes the truncation issue quantitatively small, but the lower-bound direction still has to be justified.
\end{remark}
...
\begin{theorem}[Conditional meta-theorem: certifier termination]
\label{thm:meta}
Assume:
...
\end{enumerate}
Then, conditional on these assumptions, there exists $\lambda_0$ such that for all $\lambda \geq \lambda_0$:
...
\end{enumerate}
In particular, under the same tail-bound and evaluator assumptions, there exists a sequence $\lambda_n \to \infty$ along which the hypotheses of Connes' Theorem~6.1 are verified, and by Fact~6.4 and Hurwitz's theorem, all zeros of $\Xi$ lie on the real line.
\end{theorem}
...
The certifier program (\texttt{certifier\_production.py}) implements the certified-gap computation for a geometric grid of $\lambda$-values. In the present v2.4 status this output is treated as a finite certificate \emph{conditional on} the odd-sector tail lower bound described below; the even-sector upper bound is interval-arithmetic rigorous, while the odd-sector lower-bound mechanism still requires an independent tail-bound validation in the Connes--van Suijlekom operator setting:
\begin{itemize}
  ...
  \item \textbf{Odd block (candidate lower bound on $\lambda_1^-$):} Two $N_0 \times N_0$ matrices ($N_0 = 40$--$90$) computed in float64. The smaller matrix gives a core eigenvalue $\ell_{\mathrm{core}}$; the larger provides a tail correction via the eigenvalue drop $\ell_{\mathrm{core}} - \ell_{\mathrm{big}}$. The remaining tail (modes $> N_0$) is estimated by a geometric extrapolation of coupling-column norms, and a float64 rounding margin ($10 \times N_0 \times \varepsilon_{\mathrm{mach}} \times \|W\|_\infty$) is subtracted. If this tail estimate is validated, the required lower-bound direction is
  \[
    \lambda_1^- \geq \ell_{\mathrm{big}} - \mathrm{remaining} - \varepsilon_{\mathrm{margin}}.
  \]
\end{itemize}
The gap quantity is $\mathrm{cert\_gap} = \mathrm{upper}_{\cos} - \mathrm{lower}_{\sin}$; $\mathrm{cert\_gap} < 0$ rigorously implies $\lambda_1^+ < \lambda_1^-$ only after the odd-sector lower bound $\lambda_1^- \geq \mathrm{lower}_{\sin}$ has been validated. Until then these values are strong numerical evidence and a conditional finite bridge, not a final CAP proof.

\paragraph{Operator-frame caveat (strict review 2026-06-29).}
The finite CAP and Direct Frontier diagnostics in this part are retained as
legacy/variational evidence unless and until they are replayed in the corrected
Connes--van Suijlekom $U_n$ frequency-reflection framework. They are not
theorem-level eigenvalue-ordering certificates in their present form.
...
\noindent The observed convergence of the odd block motivates the tail-bound procedure. The remaining drop from $N = 40$ to $N = \infty$ is currently controlled by geometric extrapolation of coupling-column norms (\S\ref{sec:certificates}); this extrapolation is the finite-range lower-bound component that must be replaced or independently validated for a theorem-level CAP certificate.
\end{remark}
...
For $j \geq 2$, orthogonality at $\delta = 0$ and the mean value theorem give
$|S^{\pm}(m,j;\delta,L)| \leq \pi\,\delta/L$.
Thus the mode-$j$ coupling is bounded as
\[
...
\begin{corollary}[M1$''$ variational reduction --- explicit threshold]
\label{cor:M1-proved}
...
It remains to verify that the PNT transfer error is smaller than this margin. Using the explicit Dusart bound \cite{Dusart2010} $|\theta(x) - x| < x/(20\log x)$ for $x \geq 32{,}299$, we apply the per-term relative-perturbation estimate derived in \Cref{rem:C-constant-caveat}: at $L = 13$ the error in \Cref{lem:pnt-transfer} satisfies
\[
  |\rho(\lambda) - \rho^{\mathrm{EM}}(\lambda)| \leq \frac{C(L)}{20\,L}, \qquad C(L) \;=\; \frac{3\,R(L) + 2\,|\rho^{\mathrm{EM}}(L)|}{(1 - 1/(20L))^2},
\]
where $R(L) = (E_{\sin}^{\mathrm{EM}} + E_{\cos}^{\mathrm{EM}}) / D^{\mathrm{EM}}$ is the interval-arithmetic-certified ratio sum. Explicit evaluation at $L = 13$: $R(13) = 0.6151$, $|\rho^{\mathrm{EM}}(13)| = 0.01086$, which yields $C(13) \leq 1.874$. Consequently
\[
  |\rho(\lambda) - \rho^{\mathrm{EM}}(\lambda)| \leq \frac{1.874}{20 \cdot 13} \approx 0.00721 < |\rho^{\mathrm{EM}}(13)| = 0.01086,
\]
so $\rho(\lambda) < \rho^{\mathrm{EM}}(13) + 0.00721 = -0.00365 < 0$ for all $\lambda \geq e^{13} = 442{,}413$. Since $\rho < 0$ means $E_{\sin} < E_{\cos}$, the claim follows with $\lambda_0 = 442{,}413$. This threshold lies well within the overlap zone with Regime~1 (certificates up to $\lambda = 1{,}300{,}000$), providing a safety margin of nearly one order of magnitude. For $L > 13$ the margin grows: the transfer error $C(L)/(20L)$ decreases (as $L^{-1}$ times a slowly varying factor) while $|\rho^{\mathrm{EM}}(L)|$ grows, so the required inequality $C(L)/(20L) < |\rho^{\mathrm{EM}}(L)|$ holds uniformly for all $L \geq 13$.
\end{proof}

\begin{remark}[Status of the constant $C(L)$ in the PNT transfer]
\label{rem:C-constant-caveat}
This remark concerns the three-regime proof only; the Direct Frontier-Dominance proof of \Cref{prop:direct-frontier} (v2.1) does not use the PNT transfer lemma and is independent of $C(L)$.

The bound in the proof above is \emph{not} a uniform constant $C < 2$, contrary to the heuristic formulation used in earlier versions. A rigorous derivation shows that $C(L)$ depends on $L$ and in fact grows linearly for large $L$ (e.g.\ $C(50) \approx 3.08$, $C(100) \approx 5.6$). What remains uniform and sufficient is the inequality
\[
  \frac{C(L)}{20\,L} \;<\; |\rho^{\mathrm{EM}}(L)| \qquad \text{for all } L \geq 13,
\]
which is what the proof actually requires. Numerical values: margin at $L = 13$ is $+0.00365$; at $L = 18$ it is $+0.022$; at $L = 50$ it is $+0.52$. The margin grows monotonically with $L$, so once the threshold $L = 13$ is cleared, the argument extends automatically. The remaining technical refinement is a fully interval-arithmetic-certified uniform-in-$L$ verification of the ratio sum $R(L)$ beyond the 6-mode truncation $\{\cos: 0,2,3;\, \sin: 1,2,3\}$; the tail contribution is bounded by $\leq 0.03$ via the Lemma~B decay estimate and does not affect $\lambda_0$.
\end{remark}

\begin{proposition}[Cumulative step A6]
\label{prop:A6}
The present three-regime and direct-frontier arguments establish the asymptotic
variational statement
\[
  \lambda_{\min}(W^+_\lambda-W^-_\lambda)=-\Theta(\sqrt{\lambda})
\]
and a finite bridge conditional on the odd-sector tail lower bound.  The
eigenvalue-ordering statement
$\lambda_1(\mathrm{QW}_\lambda^{\mathrm{even}})
<\lambda_1(\mathrm{QW}_\lambda^{\mathrm{odd}})$ remains conditional on the
Asymptotic Variational Gap Conjecture, or equivalently on a validated lower
bound for $\lambda_1^-(\lambda)$.
\end{proposition}

\begin{proof}[Status analysis of the three-regime argument]
\emph{Regime~1} ($\lambda \in [100,\, 1{,}300{,}000]$). The $33$ computer-assisted certificates (\Cref{sec:certificates}), computed with interval arithmetic (mpmath.iv, 50-digit precision), provide rigorous upper bounds on $\lambda_1^+$ and candidate lower bounds on $\lambda_1^-$ at each value. All recorded gaps are strictly negative. These data constitute a conditional finite bridge: they become theorem-level even-dominance certificates once the odd-sector tail lower bound used to define $\mathrm{lower}_{\sin}$ is independently validated.
...
\noindent These mechanisms support the observed stability of the finite bridge. They should not be read as a closed interpolation theorem until the required sector lower bounds are available uniformly on the intervals $[\lambda_k,\lambda_{k+1}]$.

\emph{Regime~2} ($\lambda \geq 442{,}413$). \Cref{cor:M1-proved} establishes $\rho(\lambda) < 0$ for all $\lambda \geq \lambda_0$ (via the Euler--Maclaurin Proposition, now certified by interval arithmetic at $L = 13$, and the PNT Transfer Lemma). This gives the correct asymptotic variational sign,
\[
  \lambda_{\min}(W^+_\lambda-W^-_\lambda)<0,
\]
but the implication from this variational sign to
$\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$ requires the missing odd-sector
lower bound. The higher-mode contributions are controlled by
\Cref{lem:higher-mode-decay} ($= O(D/L) = o(D)$), and the truncation correction
by \Cref{lem:resolvent-truncation} ($= o(D)$ for $\lambda \geq 500$), subject
to the same spectral-direction caveat.

\emph{Overlap.} Regimes~1 and~2 overlap in $[442{,}413,\, 1{,}300{,}000]$. Thus the remaining issue is not a coverage gap in $\lambda$ but the spectral-direction gap from a variational inequality to the eigenvalue ordering.
\end{proof}

% =============================================================================
\subsection{Direct Frontier-Dominance Proof (v2.1, robust form)}
...
\begin{proposition}[Legacy Direct Frontier-Dominance (conditional variational)]
\label{prop:direct-frontier}
For all sufficiently large $\lambda$, the legacy three-mode difference model
satisfies $\lambda_{\min}(D_{\mathrm{total}}(\lambda)) < 0$. The extension down
to $100\leq\lambda\leq\Lambda_*$ uses the finite-range diagnostics as a
conditional bridge, not as an unconditional CAP theorem.
\end{proposition}

\begin{proof}[Proof of \Cref{prop:direct-frontier}]
Decompose the prime contribution at shift order $m=1$ into the frontier window $\lambda^{0.7} \leq p \leq \lambda$ and its complement:
\[
...
\]
and $D_{\mathrm{non}}$ collects the remaining contributions (small primes $p < \lambda^{0.7}$ at shift-order $m=1$, together with all higher shift orders $m \geq 2$).

Let $e_1=(0,1,0)^\top$ denote the common middle-mode vector in the $3\times3$ shift block. The variational inequality gives
\begin{align}
...
\smallskip
\noindent\textbf{Frontier scaling (common Rayleigh vector + PNT).} On the frontier window $r \in [0.7,1]$ the fixed diagonal Rayleigh quotient is
\[
...
\smallskip
\noindent\textbf{Non-frontier scaling (PNT + Mertens).} Let $C_{\mathrm{norm}} := \max_{r \in (0, 2)} \|D_3(r)\|_{\mathrm{op}} = 2.2847$. The non-frontier weight splits into small primes and higher shift orders:
\begin{align*}
...
\smallskip
\noindent\textbf{Dominance.} Combining~\eqref{eq:direct-bound} with the two estimates,
\[
...
+ O(\log\lambda).
\]
Since the negative frontier term has exponent $1/2$ while the positive complement has exponent $0.35$, the right-hand side is strictly negative for all sufficiently large $\lambda$. Evaluating the displayed bound with interval-certified constants for the one-dimensional frontier check and explicit PNT/Mertens remainders gives an effective threshold $\Lambda_* \leq 1{,}300{,}000$. The finite interval $100 \leq \lambda \leq \Lambda_*$ is covered only conditionally by the finite-range diagnostics of \Cref{sec:certificates}. Thus the asymptotic variational sign is robust in the legacy model, while the all-$\lambda\geq100$ and eigenvalue-ordering readings require the odd-sector lower-bound validation and corrected $U_n$ replay.
\end{proof}

\begin{remark}[What the direct proof uses and avoids]
\label{rem:direct-proof-ingredients}
\Cref{prop:direct-frontier} uses only: (i) the explicit $3\times3$ shift block formula and the common Rayleigh vector $e_1$ to bound $d_{11}(r)$ on the frontier window $r \in [0.7,1]$; (ii) the PNT-based partial summation estimate for $W_f(\lambda)$; (iii) Mertens's theorem for the non-frontier sum $W_n(\lambda)$; (iv) the Rayleigh-quotient / variational inequality~\eqref{eq:direct-bound}; and (v) finite CAP evidence up to the explicit asymptotic threshold. It does \emph{not} require adding minimum eigenvalues of non-commuting summands, Regime~1 certificate-anchored interpolation (\Cref{rem:regime1-caveat}), the uniform-in-$L$ PNT-transfer constant bound (\Cref{rem:C-constant-caveat}), eigenvector tracking across the prime-induction sequence, or Hellmann--Feynman $L$-monotonicity between consecutive primes. Thus both old technical caveats of the three-regime proof are obviated for the variational statement, while the eigenvalue-ordering step remains conditional on the odd-sector lower bound.

Numerical verification of the frontier/non-frontier split across $9{,}705$ prime additions for $\lambda \in \{100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000\}$ found zero exceptions: the worst-case Rayleigh contribution $v^\top W_{p} v$ of a frontier prime is $-0.4219$ at $\lambda = 200$ and $-0.0798$ at $\lambda = 50{,}000$, consistent with the $\lambda^{-1/2}$ shrinking of per-prime contributions against a $\sqrt{\lambda}$-growing aggregate.
\end{remark}

\begin{remark}[Rigorous status of Regime~1 interpolation, v2.0 three-regime proof]
\label{rem:regime1-caveat}
This remark documents the rigorous status of the three-regime proof as it stood in v2.0 and is retained for historical accuracy. The Direct Frontier-Dominance proof (\Cref{prop:direct-frontier}, added in v2.1) does not depend on Regime~1 interpolation and obviates the caveat below.

The CAP diagnostics establish $\mathrm{cert\_gap}(\lambda_k) < 0$ at the $33$ grid points $\lambda_k$ conditional on the odd-sector lower-bound validation described in \Cref{sec:certificates}. Extending this to every $\lambda \in [\lambda_k, \lambda_{k+1}]$ uses three ingredients (i)--(iii) above. Of these, (i) (Rayleigh-quotient perturbation under rank-one updates) and (iii) (eigenvector stability via the certified intra-sector gap) are rigorous \emph{provided} $\|W_p\|_{\mathrm{op}} < 0.46$ remains bounded on the interval and the intra-even gap remains above the interval-valued perturbation radius; both are certified at the grid points, not continuously. Between certificate points the argument is therefore \emph{quasi-rigorous}: it rests on the assumption that the certified intra-sector gap varies continuously and does not collapse on the (finitely primed) segments $[\lambda_k, \lambda_{k+1}]$ --- a property that is numerically confirmed at the dense-grid resolvent level but not derived as a closed-form inequality. Ingredient~(ii) (Hellmann--Feynman on $L$-variation between consecutive primes) is analytically derivable from the kernel structure but has been tabulated only up to $\lambda = 200$ in Table~\ref{tab:hf-results}; its extrapolation to $\lambda \leq 1{,}300{,}000$ relies on the kernel's smooth $L$-dependence. In v2.0 Regime~1 is therefore \emph{certificate-anchored with perturbative interpolation}. The Regime~2 argument (via \Cref{cor:M1-proved}) supplies the asymptotic variational sign beyond $\lambda_0 = 442{,}413$; the v2.1 Direct Frontier-Dominance proof is independent of the interpolation and of the Regime~2 PNT-transfer constant, while still using the finite diagnostics as a conditional bridge below its explicit asymptotic threshold.
\end{remark}

\begin{remark}[Status of the proof, revised v2.2]
\label{rem:M1-status}
\Cref{prop:A6} provides \emph{two parallel arguments}, both of which establish
the variational statement $\lambda_{\min}(W^+-W^-)=-\Theta(\sqrt\lambda)$
asymptotically but not, by themselves, the eigenvalue inequality
$\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$:
\begin{itemize}
  \item \textbf{v2.0 three-regime argument} (above): combines the $33$ finite-range diagnostics (Regime~1), the M1$''$ resolvent framework with PNT transfer (Regime~2), and the higher-mode / truncation control (\Cref{lem:higher-mode-decay,lem:resolvent-truncation}). Two technical caveats (\Cref{rem:regime1-caveat,rem:C-constant-caveat}) describe the remaining rigorous gaps in this argument.
  \item \textbf{v2.1 Direct Frontier-Dominance proof} (\Cref{prop:direct-frontier}, \Cref{sec:direct-frontier}): uses a common Rayleigh test vector on $r\in[0.7,1]$, PNT partial summation, Mertens's theorem, the variational inequality, and finite diagnostic coverage up to the explicit asymptotic threshold. Both v2.0 caveats are obviated for the variational statement (\Cref{rem:direct-proof-ingredients}).
\end{itemize}

\medskip\noindent
\textbf{v2.2 correction: variational direction gap.}  Both proofs above
work with $\lambda_{\min}(W^+-W^-)$, equivalently with the Rayleigh
quotient $v^T(W^+-W^-)v$ at a test vector~$v$ (e.g.\ $v=e_1$ in v2.1).
This shows the existence of a vector $v$ with $v^T W^+ v < v^T W^- v$,
but not $\lambda_{\min}(W^+) < \lambda_{\min}(W^-)$, because the
Rayleigh inequality $\lambda_{\min}(W^-) \le v^T W^- v$ runs in the same
(upper-bound) direction.  The companion paper \cite{Geiger2026evendom}
formulates the missing lower bound on $\lambda_1^-(\lambda)$ as the
\emph{Asymptotic Variational Gap Conjecture} and sketches a proposed
route via degenerate perturbation theory on the asymmetric three-mode
diagonal structure.

\textbf{Revised status of the A1--A8 chain.}
Together with Connes' Theorem~6.1 (A1), Hurwitz sufficiency (A2), the
Shift Parity Lemma (A4), the IA-certified finite-range even dominance
(A3), and the frontier-prime mechanism (A5), the present paper
establishes a \emph{conditional reduction} of RH:
\begin{itemize}
  \item \textbf{Unconditional outputs:} Shift Parity Lemma; finite-range
    negative gap diagnostics on $100 \le \lambda \le 1{,}300{,}000$
    conditional on the odd-sector tail lower bound; Leading-Mode
    Cancellation Lemma; non-existence theorem NE-A and the finite-dimensional
    NE-B computation.
  \item \textbf{Conditional outputs:} Asymptotic even dominance for
    $\lambda \ge \Lambda_*$ is conditional on the Asymptotic Variational
    Gap Conjecture for the odd-sector minimum eigenvalue.  RH itself is
    additionally conditional on the Connes program (Theorem~6.1 and
    Hurwitz bridge in~\cite{Connes2026, ConnesvS2025}).
\end{itemize}
The status claim of v2.1 (``unconditional A1--A8'') is hereby revised.
Drafts and preprints are continuously evolving artefacts; v2.2 is the
current honest position.

\medskip\noindent
\textbf{v2.3 update (15 May 2026): Connes-2026 Fact~6.4 conditioning
clarified.}  A close reading of Connes 2026 §6.5 \cite{Connes2026}
clarifies that the Hurwitz-bridge (A2) is presented there as
\emph{Fact~6.4}, an analytical consequence of the classical
Slepian--Pollak prolate spheroidal wave function convergence theory
\cite[Theorem~1 of reference~47]{Connes2026}, not as a conjecture or
unproven strategy.  The ``proof strategy'' language in the abstract
of \cite{Connes2026} refers to the global RH-architecture
(combining Fact 6.4 with the two remaining steps in §6.6:
even-ground-state simplicity and approximation quality
$k_\Lambda\approx\xi_\Lambda$), not to Fact 6.4 itself.  Therefore the
correct conditioning for the A1--A8 chain is:
\begin{itemize}
  \item \emph{External established input:} Connes--van Suijlekom
    real-zero criterion \cite{ConnesvS2025} (proved); Fact~6.4
    Fourier convergence \cite[§6.5]{Connes2026} (established via
    Slepian--Pollak).
  \item \emph{Internal open question:} simple even ground state of
    $QW_\Lambda$ (Connes 2026 §6.6 item (i)) = Asymptotic Even
    Dominance --- the central goal of the present paper and of the
    companion \cite{Geiger2026evendom}.
  \item \emph{Internal open question (separate route):}
    approximation quality $k_\Lambda \approx \xi_\Lambda$
    (Connes 2026 §6.6 item (ii)) = MS2, addressed in the companion
    Zookeeper paper \cite{Geiger2026zookeeper}.
\end{itemize}
The two ``remaining steps'' Connes identifies in \cite[§6.6]{Connes2026}
are therefore exactly the two open questions our two main papers
attempt to discharge.

The spectral gap bound in \Cref{lem:higher-mode-decay} is derived analytically via the parity-split Laplace asymptotics (Step~3): $c_{\mathrm{gap}} \geq 4\pi^2/L \geq 2.8$ for $L \leq 14$, conservative by a factor $\geq 5$ compared to the earlier numerical estimate $c_{\mathrm{gap}} \geq 0.5$. Combined with the sector-difference control (\Cref{lem:resolvent-truncation}), no numerical constants remain in the Regime~2 argument. In the v2.1 Direct Frontier-Dominance proof, the numerical inputs are fixed-domain interval certificates independent of $\lambda$: $C_{\mathrm{front}} = 0.4685$ for the common frontier Rayleigh quotient and $C_{\mathrm{norm}} = 2.2847$ for the non-frontier norm bound.
\end{remark}
