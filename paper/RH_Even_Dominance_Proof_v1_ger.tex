...
Folgend auf Connes' Reduktion der Riemannschen Vermutung auf eine Bedingung der geraden Dominanz für die Weilsche quadratische Form leisten wir zwei unabhängige Beiträge. Erstens ist für den endlichen Bereich $100\leq\lambda\leq 1{,}300{,}000$ die gerade Dominanz $\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$ rigoros zertifiziert: intervallarithmetische Galerkin-Schranken liefern an jedem Zertifikatspunkt eine separate obere Schranke für $\lambda_1^+$ und eine separate untere Schranke für $\lambda_1^-$. Zweitens wird für das asymptotische Regime $\lambda\to\infty$ über ein gemeinsames Rayleigh-Vektor-Argument gezeigt, dass $\lambda_{\min}(W^+_{\mathrm{prime}}-W^-_{\mathrm{prime}})=-\Theta(\sqrt\lambda)$ auf dem relevanten Drei-Modus-Galerkin-Block gilt. Die Lücke zwischen dieser Variations-Aussage und der eigentlichen Eigenwert-Anordnung $\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$ im asymptotischen Bereich erfordert eine separate quantitative untere Schranke für $\lambda_1^-$; wir formulieren dies als \emph{Asymptotische Variationsgap-Vermutung} und skizzieren in \Cref{sec:asymptotic-lower-bound} eine Route über entartete Störungstheorie.
Der hier präsentierte Beweis der Riemannschen Vermutung ist daher bedingt auf (i) Connes--van Suijlekoms Reell-Nullstellen-Kriterium und die Hurwitz-Brücke aus~\cite{Connes2026}, sowie (ii) die Asymptotische Variationsgap-Vermutung für den minimalen Eigenwert im ungeraden Sektor. Das Shift-Parity-Lemma und die intervallarithmetischen Zertifikate bleiben unbedingt. Die ausgeschlossenen Eich-, Total-Positivitäts- und universellen Kommutator-Routen werden nicht verwendet und es wird auf die Landscape-Reihe verwiesen.
\end{abstract}
...
\[
\begin{gathered}
\text{Shift-Parität}+\text{IA-Zertifikate für }100\leq\lambda\leq 1{,}300{,}000\\
\Longrightarrow\ \text{gerade Dominanz auf dem zertifizierten Bereich (\Cref{thm:even-finite})},
\end{gathered}
\]
zusammen mit
\[
...
\end{gathered}
\]
Die erste Zeile ist unbedingt. Die zweite Zeile führt zwei bedingte Eingaben ein: das Connes-Programm (extern) und die Asymptotische Variationsgap-Vermutung (\Cref{conj:avg}, intern zu diesem Ansatz; vgl.\ \Cref{sec:asymptotic-lower-bound}).
...
\begin{lemma}[Single-Shift-Asymmetrie; v1.5-Analog des Shift-Parity-Lemmas]
...
\paragraph{v1.5-Status der CAP-Konstruktion.}  Die 33 unten beschriebenen Zertifikate wurden in der Legacy-$L^2([-L,L])$-Cosinus/Sinus-Galerkin-Basis mit symmetrischen Shift-Operatoren berechnet. Das v1.5-Audit (\Cref{sec:k0-strengthened}; Begleitnotizen \texttt{K0\_CAP\_AUDIT.md} und \texttt{K0\_CAP\_CONNESVS\_RESULTS.md}) zeigt zwei strukturelle Probleme: (1) Im Legacy-Operator-Modus erzwingt die Cosinus-Sinus-Aequivalenz $\sigma(A^+) = \sigma(A^-) \cup \{\mu^+_0\}$ exakt, was den literalen Zertifikat-Anspruch invalidiert (in der Connes-vS-Form aufgeloest). (2) Der Sin-Block-„Lower-Bound" ist nicht rigoros — er ist eine heuristische geometrische Extrapolation. Die Zertifikate stehen daher als \emph{computergestuetzte numerische Evidenz}, nicht als rigoroser CAP-Beweis. Eine v1.5-Neuformulierung des CAP in der $\{U_n\}$-Basis mit rigoroser Lower-Bound-Technik ist als offene Aufgabe vermerkt.
...
\paragraph{Zusammenfassung der v1.5-Reformulierung.}  In der Connes--van-Suijlekom-Form von Section~2 ist das Analog der Frontier-Dominanz die asymptotische Positivität des Cos/Sin-Sektor-Asymmetriekoeffizienten $S_n$ für jeden Mode~$n$. Für $n=1$ liefert eine Abel-Summation und Laplace-Asymptotik
\begin{equation}
\label{eq:S1-asymptotic-cvs-de}
  S_1(\lambda) \;\sim\; \frac{8\sqrt\lambda}{\log\lambda}
  \quad (\lambda\to\infty),
\end{equation}
mit der genaueren finite-$L$-Form $S_1(\lambda) \approx 8\sqrt\lambda/[L(1+16\pi^2/L^2)]$, wobei $L=\log\lambda$. Numerisch verifiziert auf $5{-}10\%$ Genauigkeit für $\lambda\in\{100,300,1000\}$ (Skript \texttt{\_proof-notes/verify\_connes\_vs\_form.py}; Detail in \texttt{\_proof-notes/K0\_FRONTIER\_SN\_ASYMPTOTIC.md}). In dieser v1.5-Form ist die Cos-minus-Sin-Diagonal-Lücke am Mode~$n=1$:
\[
  \langle\sin_1\,|\,Q\,|\,\sin_1\rangle - \langle\cos_1\,|\,Q\,|\,\cos_1\rangle = 2 S_1(\lambda) \;\sim\; \frac{16\sqrt\lambda}{\log\lambda},
\]
d.h.\ ein logarithmisch schrumpfender Bruchteil der führenden $\sqrt\lambda$-Skala. Gerade Dominanz gilt daher asymptotisch mit Margin $O(\sqrt\lambda/\log\lambda)$ statt $O(\sqrt\lambda)$, wie es das Legacy-Direct-Frontier-Argument vermuten ließ. Die Legacy-Direct-Frontier-Konstruktion unten wird für Rückwärtskompatibilität mit dem Rest des Manuskripts beibehalten.
...
\paragraph{v1.5-Reformulierungs-Hinweis.}  Dieser Abschnitt, geschrieben in der Legacy-$L^2([-L,L])$-Basis, kontrolliert die Tail-Beitraege hoeherer Moden in diesem Rahmen. In der Connes--van-Suijlekom-Form von Section~2 ist die analoge Aufgabe die Kontrolle der Off-Diagonalen $\langle U_m | Q | U_n\rangle$ fuer $|m|, |n| > N$. Die unten gegebenen Abschaetzungen uebertragen sich, mit der Modifikation dass die relevanten Ueberlapp-Funktionen $\sin(2\pi m y/L) - \sin(2\pi n y/L)$ statt $D_3(r)$ sind. Insbesondere ersetzt die subleading-Asymptotik $S_n \sim 8\sqrt\lambda/[L(1+16n^2\pi^2/L^2)]$ den fuehrenden $\Theta(\sqrt\lambda)$-Beitrag des Legacy-Direct-Frontier-Bound, was eine $O(\sqrt\lambda/\log\lambda)$-Skala fuer die Cos-Sin-Diagonal-Luecke gibt.
...
\paragraph{v1.5-Status des Haupttheorems.}  In der Connes-vS-Form von Section~2 ist das Haupttheorem (RH bedingt auf das Connes-Programm und die Asymptotische Variationsgap-Vermutung) unveraendert. Was sich aendert ist die praezise Formulierung der Conditioning:
\begin{itemize}[leftmargin=*]
  \item \emph{Externe etablierte Eingabe aus Connes 2026 §6.5 (Fact 6.4):} Die Fourier-Transformierte der Educated-Guess-Eigenfunktion $k_\Lambda$ konvergiert gegen $\Xi$ gleichmaessig auf Substreifen $|\Im z|<\delta<1/2$ mit Rate $O(\Lambda^{-1/2-\delta})$. Dies ist in \cite{Connes2026} als etablierte Folge der klassischen Slepian--Pollak prolaten spheroidalen Wellenfunktions-Theorie ([Theorem~1 of reference~47]{Connes2026}) formuliert, NICHT als Conjecture.
  \item \emph{Externe etablierte Eingabe aus Connes--van Suijlekom 2025 \cite{ConnesvS2025}:} Das fixed-$\Lambda$-Quadratform-Reell-Nullstellen-Kriterium, voll bewiesen.
  \item \emph{Interne offene Frage:} Einfachheit und Paritaet des minimalen Eigenwerts von $QW_\Lambda$ (Connes 2026 §6.6 ``remaining steps'' item~(i)) --- das ist Even Dominance wie in Definition~\ref{def:even-dominance} definiert, adressiert im vorliegenden Paper.
...
\begin{theorem}[Gerade Dominanz auf dem zertifizierten Bereich]
\label{thm:even-finite}
Für jedes $\lambda$ im zertifizierten Gitter aus \Cref{prop:finite-certificates} (33 Werte über $100\leq\lambda\leq 1{,}300{,}000$) hat $\QW_\lambda$ einen einfachen geraden Grundzustand, d.\,h.\ $\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$ und das gerade Minimum ist isoliert.
\end{theorem}

\begin{proof}
Die intervallarithmetischen Galerkin-Zertifikate aus \Cref{prop:finite-certificates} liefern an jedem $\lambda$ des Gitters eine rigorose obere Schranke $\overline{\lambda_1^+}(\lambda)$ und eine rigorose untere Schranke $\underline{\lambda_1^-}(\lambda)$ aus dem geraden bzw.\ ungeraden Galerkin-Block, zusammen mit Cauchy-artigen Tail-Schranken. Die zertifizierte Lücke $\overline{\lambda_1^+}(\lambda)-\underline{\lambda_1^-}(\lambda)<0$ an jedem Gitterpunkt liefert direkt $\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$ auf dem zertifizierten Bereich. Die Einfachheit folgt aus der zertifizierten Intra-Even-Lücke $\lambda_2^+-\lambda_1^+>0$ an jedem Gitterpunkt.
\end{proof}
...
\]
zeigt, dass $\lambda_{\min}(A-B)=-5<0$ nicht $\lambda_{\min}(A)<\lambda_{\min}(B)$ impliziert; tatsächlich ist $\lambda_{\min}(A)=0>-5=\lambda_{\min}(B)$. Auf dem zertifizierten Bereich wird diese Lücke durch die direkte Berechnung von $\underline{\lambda_1^-}$ in \Cref{thm:even-finite} geschlossen; asymptotisch ist ein separates Argument erforderlich.
...
\begin{proof}
\sloppy
Nach \Cref{thm:even-finite} gilt gerade Dominanz an jedem $\lambda$ im zertifizierten Gitter; nach \Cref{conj:avg} gilt sie für jedes $\lambda\geq\Lambda^\dagger$. Beides liefert eine divergente Folge $\lambda_n\to\infty$ gerade-dominanter Parameter. Wendet man \Cref{cor:sequence-suffices} auf diese Folge an: Die Fourier-Transformationen der gerade-dominanten Weil-Formen haben nach \Cref{thm:connes} ausschließlich reelle Nullstellen, und die Hurwitz-Brücke überträgt die Reell-Nullstellen-Eigenschaft auf die begrenzende $\Xi$-Funktion. Daher liegen die nichttrivialen Nullstellen von $\zeta(s)$ auf $\Re(s)=1/2$.
\end{proof}
...
\paragraph{Ehrliche Einschätzung.} Schritte~1 und~2 sind Routine, aber arbeitsintensiv (mehrere Wochen). Schritt~3 ist genuin schwer: entartete Störungstheorie mit vorzeichenkontrollierten Kopplungen ist eine etablierte Disziplin, aber jede explizite Konstante kann ein Teilproblem werden. Die Beziehung $|\lambda_1^+-\lambda_1^-|=O(\sqrt\lambda/L)$, die dieses Programm nahelegt, ist konsistent mit der numerischen Beobachtung, dass die zertifizierte Lücke etwa $20\%$ von $|\lambda_1^+|$ beträgt -- kein konstanter Bruchteil von $\sqrt\lambda$, sondern ein logarithmisch schrumpfender Bruchteil.
...
Dieses Update entspricht der Behandlung des vorliegenden Manuskripts als kontinuierlich entwickelndem Draft. Die Begleitnotizen \texttt{\_proof-notes/K0\_STEP1\_FINDINGS.md} und \texttt{\_proof-notes/K0\_CAP\_AUDIT.md} liefern den vollständigen analytischen und numerischen Befund der vorliegenden Spannung.

\subsection{Update (15. Mai 2026): Connes-vS-Form loest K0-Spannung; rigorose CAP-Schliessung bleibt offen}
\label{sec:k0-cvs-resolution}

Eine spaetere Vergleichanalyse der Operatordefinition gegen Connes--van Suijlekom \cite{ConnesvS2025} klaerte, dass die verschaerfte K0-Sorge (die spektrale Aequivalenz $\sigma(A^+) = \sigma(A^-) \cup \{\mu^+_0\}$) ein Artefakt der Legacy-Operator-Realisierung auf $L^2([-L,L])$ mit reeller Cosinus/Sinus-Basis unter $t\mapsto -t$-Symmetrie war, nicht des tatsaechlichen Connes-Operators. Der bewiesene Connes--van-Suijlekom-Operator wirkt auf $L^2([0,L])$ mit der komplexen Exponentialbasis $U_n$, und die Cosinus/Sinus-Paritaetssektoren werden ueber die Frequenzreflektion $U_n\leftrightarrow U_{-n}$ definiert. In dieser korrigierten Formulierung (Section~2 v1.5-Reformulierung) unterscheiden sich die Diagonalen $\langle\cos_n|Q|\cos_n\rangle = D_n - S_n$ und $\langle\sin_n|Q|\sin_n\rangle = D_n + S_n$ um die genuin nicht-triviale Asymmetrie $2S_n$. Direkte Berechnung (Skript \texttt{verify\_connes\_vs\_form.py}) ergibt $S_n > 0$ fuer die Weil-Distribution und alle getesteten $\lambda$.

Die verbleibende Aufgabe ist eine \emph{rigorose} CAP-Schliessung von $\lambda_1^{\cos}(\lambda) < \lambda_1^{\sin}(\lambda)$ in dieser Form. Eine systematische Analyse der verfuegbaren rigorosen Lower-Bound-Techniken ist in \texttt{\_proof-notes/K0\_LEHMANN\_ANALYSIS.md} dokumentiert: Temple's Ungleichung, Lehmann-Maehly, Bauer-Fike, Operator-Norm-Bound, Schur-Komplement und Gerschgorin scheitern alle aus einem gemeinsamen strukturellen Grund — keine a-priori Lower-Bound auf $\lambda_2$ ist verfuegbar, die Galerkin-Variationsprinzip allein nicht liefert. Drei mittelfristige Forschungsrichtungen sind dort skizziert: Mollifikation, Toeplitz-Approximation, Reverse-Engineering aus Connes--vS 2025.

\paragraph{Aktuelle ehrliche Position.}  Die Connes--van-Suijlekom-Operator-Form ist die korrekte Einstellung und zeigt gerade Dominanz numerisch mit konsistenter Margin ueber $\lambda$ und $N$. Die asymptotische $S_1$-Asymptotik gibt eine quantitative Cos-Sin-Diagonal-Luecke der Ordnung $\sqrt\lambda/\log\lambda$. Eine rigorose CAP-aehnliche Schliessung im Sinne einer zertifizierten unteren Schranke auf $\lambda_1^{\sin}$, die strikt eine zertifizierte obere Schranke auf $\lambda_1^{\cos}$ uebertrifft, bleibt offen und erfordert einen nicht-trivialen technischen Fortschritt jenseits einfachem Galerkin + Temple/Lehmann.
