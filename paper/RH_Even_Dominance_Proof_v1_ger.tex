...
\begin{abstract}
Dieses Paper präsentiert eine bedingte Reduktion der Riemannschen Vermutung, extrahiert aus einer dreiteiligen Landscape-Reihe. Die Landscape-Paper dokumentieren den vollständigen Forschungsprozess, einschließlich fehlgeschlagener Routen, Obstruktionstheoreme, rechnergestützter Zertifikate und alternativer Strategien. Das vorliegende Paper behält nur die beweisrelevante Kette bei.
Folgend auf Connes' Reduktion der Riemannschen Vermutung auf eine Bedingung der geraden Dominanz für die Weilsche quadratische Form leisten wir zwei unabhängige Beiträge. Erstens berichtet die Landscape-Rechnung für den endlichen Bereich $100\leq\lambda\leq 1{,}300{,}000$ negative intervall-eingeschlossene Galerkin-Lücken an 33 Gitterpunkten. Ein Audit vom 15. Mai 2026 zeigt jedoch, dass die aktuelle Sin-Block-Lower-Bound-Konstruktion noch eine heuristische geometrische Tail-Extrapolation enthält; die endlichen Daten werden daher hier als computergestützte numerische Evidenz behandelt, nicht als CAP-Beweis auf Theoremniveau. Zweitens wird für das asymptotische Regime $\lambda\to\infty$ über ein gemeinsames Rayleigh-Vektor-Argument gezeigt, dass $\lambda_{\min}(W^+_{\mathrm{prime}}-W^-_{\mathrm{prime}})=-\Theta(\sqrt\lambda)$ auf dem relevanten Drei-Modus-Galerkin-Block gilt. Die Lücke zwischen dieser Variations-Aussage und der eigentlichen Eigenwert-Anordnung $\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$ im asymptotischen Bereich erfordert eine separate quantitative untere Schranke für $\lambda_1^-$; wir formulieren dies als \emph{Asymptotische Variationsgap-Vermutung} und skizzieren in \Cref{sec:asymptotic-lower-bound} eine Route über entartete Störungstheorie.
Der hier präsentierte Beweis der Riemannschen Vermutung ist daher bedingt auf (i) Connes--van Suijlekoms Reell-Nullstellen-Kriterium und die Hurwitz-Brücke aus~\cite{Connes2026}, deren Fact~6.4-Konvergenz dort als Slepian--Pollak-Folge dargestellt wird, sowie (ii) die Asymptotische Variationsgap-Vermutung für den minimalen Eigenwert im ungeraden Sektor. Das Shift-Parity-Lemma bleibt unbedingt; die intervallarithmetischen Zertifikate bleiben numerische Evidenz, bis eine rigorose Lower-Bound-Schließung vorliegt. Die ausgeschlossenen Eich-, Total-Positivitäts- und universellen Kommutator-Routen werden nicht verwendet und es wird auf die Landscape-Reihe verwiesen.
\end{abstract}
...
Im korrigierten Setup von Section~2 trägt eine einzelne Dirac-Masse bei $y\in(0,L)$ mit Gewicht $w$ in der Weil-Distribution $D$ zu den Diagonalen bei:
\[
...
In der Connes--van-Suijlekom-Form von Section~2 ist das Analog der Frontier-Dominanz die asymptotische Positivität des Cos/Sin-Sektor-Asymmetriekoeffizienten $S_n$ für jeden Mode~$n$. Für $n=1$ liefert eine Abel-Summation und Laplace-Asymptotik
\begin{equation}
...
\begin{proof}
Die Schur-Komplement-Schranke ist Standard~\cite{Kato1966}. \Cref{lem:higher-mode}(i) liefert $\norm{B_{\mathrm{tail}}}_{\mathrm{op}}\leq 8\pi\sqrt\lambda/L$. \Cref{lem:higher-mode}(ii) liefert $g_{N_0+1}\geq 4\pi^2 N_0^2\sqrt\lambda/L^2 \gg \norm{B_{\mathrm{tail}}}_{\mathrm{op}}$ für $N_0\geq 40$, weshalb der Nenner von $g_{N_0+1}$ dominiert wird. Die gezeigte Schranke $16\sqrt\lambda/N_0^2$ folgt daraus. Die berichtete Lücke nutzt den schärferen intervallarithmetischen Einschluss aus \cite[Section~2.9]{GeigerLandscapeII} mit der diagnostischen Cauchy-Tail-Schranke, die in \Cref{rem:truncation-safety} beschrieben ist.
\end{proof}

\section{Haupttheorem und offene Vermutung}

\paragraph{v1.5-Status des Haupttheorems.}  In der Connes-vS-Form von Section~2 ist das Haupttheorem (RH bedingt auf das Connes-Programm und die Asymptotische Variationsgap-Vermutung) unverändert. Was sich ändert, ist die präzise Formulierung der Conditioning:
...
\label{thm:rh}
Angenommen, \Cref{conj:avg} (Asymptotische Variationsgap) und das Connes-Programm, namentlich das Reell-Nullstellen-Kriterium für quadratische Formen aus~\cite{ConnesvS2025} zusammen mit der Hurwitz-Brücke aus \cite[Fact~6.4 und Section~6.4--6.6]{Connes2026}, gelten. Dann liegen alle nichttrivialen Nullstellen der Riemannschen Zetafunktion~\cite{Riemann1859} auf der kritischen Geraden $\Re(s)=1/2$.
\end{theorem}
