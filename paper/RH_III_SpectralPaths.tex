...
% =============================================================================
\section{Update (v3.2): Closed Units, June--August 2026}
\label{sec:v32-update}
% =============================================================================

Between June and August 2026 a series of finite, audit-carrying working
units was closed on and around the spectral sector. They are recorded here
in the same spirit as the exclusion catalogues above: each entry states
what was examined, how far it was verified, what it means for the route,
and --- explicitly --- what it does \emph{not} claim. None of them closes
RH, and none changes the conditional-reduction status of Branches~B and~Z
established in the preceding sections.

\begin{remark}[Reading the counts in this section]
\label{rem:v32-counts}
Two kinds of ratio appear below and must not be read alike. A
\emph{verification quota} --- ``producer $18/18$'', ``independent verifier
$22/22$'' --- reports checks that were carried out and passed. A
\emph{shortfall count} reports how many required items are \emph{absent}:
where the text below says that none of fifteen prerequisites is met, that
none of fourteen proof-carrying categories is present, or that none of
twenty-one packages exists, these are missing-item statements and
emphatically not pass rates. For that reason every shortfall is written
out in words, while genuine verification quotas are given as ratios and
named as such.
\end{remark}

\subsection{Block A: The Freedman/Weyl Continuum Route}
\label{sec:v32-block-a}

\begin{itemize}
  \item \textbf{Quotient-to-Weyl commitment contract V1 (exact finite
    contract with counter-model).} The unit fixes contractually what a
    lift from finite quotient positivity to the Weyl/continuum statement
    must deliver, and secures this by a counter-model that blocks the
    inference from positivity on $\ker R$ to full positivity. Verification:
    producer $14/14$, independent $16/16$, with a byte-level attack on the
    bound sources failing closed. A later reading of the project register
    (state~V1.1, more recent than the plan state) sharpens the picture: the
    source discharges the boundary obligations itself ---
    $\beta_{\mathrm{bdy}} = 0$ and $D_{\mathrm{bdy}} = 0$ by a primitive
    endpoint computation, and $D_q|_Y = 0 \Longleftrightarrow D_q = 0$ on
    $X_R$ by density --- and states the equivalence
    $D_q = 0 \Longleftrightarrow \Gamma^*\Gamma \le C
    \Longleftrightarrow Q_\Phi \ge 0$ as a direct lift theorem of the
    original. For the route map this means that, on register state~V1.1,
    the Weyl lift is no longer a diffuse list of seven continuum
    obligations, but is concentrated on a
    single analytical residual obligation: $\Gamma^*\Gamma \le C$ on $X_R$,
    uniformly in $\omega$, together with the framing obligations. The
    finite quotient finding alone still does not carry the lift. Claim
    boundary: no PASS is asserted for $\Gamma^*\Gamma \le C$, for the lift,
    for Weyl, for KLM or for RH, and the earlier formulation ``seven
    continuum obligations open'' is superseded by register state~V1.1.

  \item \textbf{Uniform-$\omega$ reweighting audit V1 (finite negative
    finding).} Positive and locally uniformly integrable branch weights do
    \emph{not} preserve the aggregated square domination without an
    additional hypothesis. Verification: producer $15/15$, independent
    $17/17$, fail-closed. Because the finding is finite and exact rather
    than numerical evidence, it is usable directly as a route decision. Its
    consequence is that the obvious reweighting strategy for
    $|\omega| < 1/2$ dies; what remains open is the operator-bound
    alternative (branchwise domination, a uniformly weighted intertwiner, a
    conjugated contraction, and a parameter cover with positive margin).
    Claim boundary: this is \emph{no} counterexample to the actual Freedman
    kernel --- what is refuted is the strategy, not the statement.

  \item \textbf{Endpoint-to-continuum follow-on chain validation and DAG
    audit (negative).} Eleven dashboard rows were reduced to four
    source-coherence cuts, and the follow-on chain endpoint $\to$ rank
    $\to$ Green $\to$ unique continuation $\to$ active range was validated
    negatively. The bound sources carry no mutation counts for this unit,
    and none are supplied here. The result is not a universal no-go: it is
    a negative finding for exactly this chain and for no wider claim. As a
    by-product, the orphan-file cross-reference was closed as a finite
    diagnostic instrument. Claim boundary: neither the endpoint layer nor
    the continuum target is refuted; what fails is the specific chaining of
    the two.

  \item \textbf{All-time/continuum and pullback readiness audit V1
    (fail-closed).} Fifteen prerequisites of an all-time-capable,
    continuum-capable and non-circular KLM $\to$ de~Branges route were
    audited, and \emph{none of the fifteen is met} --- a shortfall count in
    the sense of \Cref{rem:v32-counts}, not a pass rate. The audit itself
    is clean and carries its own quotas: producer $18/18$, independent
    verifier $22/22$, terminal status
    \texttt{NOT\_\allowbreak READY\_\allowbreak MISSING\_\allowbreak
    BOUND\_\allowbreak CONTINUUM\_\allowbreak AND\_\allowbreak
    PULLBACK\_\allowbreak OBJECTS\_\allowbreak NEW\_\allowbreak
    MATHEMATICAL\_\allowbreak INPUT\_\allowbreak REQUIRED}. In this path no
    autonomously executable static or finite computational step remains, so
    movement requires new mathematical input rather than further
    computation. Claim boundary: the bridge is unfinished, not refuted ---
    the audit measures readiness, not truth.
\end{itemize}

\subsection{Block B: The KLM/de~Branges Route}
\label{sec:v32-block-b}

\begin{itemize}
  \item \textbf{KLM $\to$ de~Branges pullback-obligation audit V1
    (cross-term counter-model).} From separately nonnegative tail and
    boundary differences, full signed Gram positivity does \emph{not}
    follow. Verification: producer $17/17$, independent $19/19$,
    fail-closed. The route consequence is that the ``separately nonnegative
    suffices'' strategy dies, and the untreated cross terms are thereby
    identified as the decision locus for any successor attempt. Claim
    boundary: the actual shifted-$\Xi$ kernel is not refuted; what falls is
    the sufficiency argument, and it falls to an explicit counter-model
    rather than to a numerical margin.

  \item \textbf{KLM support, isometry and defect-lift layer (four closed
    findings).} On the positive side, a weighted full-line support $H_\rho$
    realises both Mellin halves (producer $28/28$, independent $34/34$),
    and a target-independent fixed-trace support $\mathrm{Mu}$ was
    constructed (producer $19/19$; verifier with $49$ direct checks plus
    $21/21$). On the negative side, no scalar $a_t T_t$ is isometric for
    $t \neq 0$ and an exact weight cocycle destroys the $k_z$ eigenrelation
    (producer $22/22$, independent $28/28$); a universal joint cross-Gram
    repair of the defect lifts $J_{(\sigma,u)}$ is refuted (producer
    $26/26$, independent $31/31$, with $126+378$ controls); and the
    $(\Lambda_a, \mathrm{Mu}_z)$ nullspace annihilation is vacuous from
    saturation onwards (producer $17/17$, independent $21/21$). The route
    reading is that the supports exist, while the isometry route and the
    universal defect-lift repair route are closed --- which also devalues
    an earlier item that had been recorded as progress. Claim boundary:
    each negative statement is bound to its stated class (scalar
    isometries, exact cocycles, universal joint repairs), and none of them
    refutes the KLM pullback target itself.

  \item \textbf{Complete normalised KLM residual block --- directed
    non-PSD (final finding).} Five preregistered Hardy nodes and all ten
    Riesz vectors were taken from directed normalised $79\times 10$ branch
    maps, and the Hardy target block was re-enclosed with Arb/Acb ($20$
    modes, majorants below $2^{-120}$). For the residual block
    $\Delta = H - C$ all ten real diagonal boxes have strictly negative
    upper bounds, the largest being $\approx -0.2665171987\ldots$.
    Verification: $26/26$ sources byte-bound, $384$-bit producer $17/17$,
    independent $512$-bit verifier $20/20$, fail-closed. This joint
    branch-symmetric five-node polar and graph normalisation therefore dies
    rigorously as a positive pullback candidate, as an endpoint finding
    rather than a provisional one; two coarser variants had already been
    excluded numerically beforehand (plus-matching $(5,0,5)$ with
    $\lambda_{\min} \approx -0.11543$, unit direct sums $(4,1,5)$ with
    $\lambda_{\min} = -12.2767\ldots$). Claim boundary: the result binds
    this one finite candidate; the Mellin split is not refuted by it.

  \item \textbf{Operator-semantic alternatives audit V1.} Three admitted
    classes were examined for a target-free alternative: branch-weighted
    graph metrics, fixed trace augmentations, and coupled defect channels.
    None of the three yields a unique target-free choice, and no matrix was
    produced. Verification: producer $15/15$, verifier $17/17$, terminal
    status \texttt{NO\_\allowbreak TARGET\_\allowbreak FREE\_\allowbreak
    ...
    CONTINUUM\_\allowbreak PULLBACK}. The route consequence is that the
    finite matrix path is exhausted, which is what routes this line into
    the readiness audit of Block~A above. Claim boundary: exhaustion is
    asserted for the three admitted classes only --- a class outside that
    admission is untested rather than excluded.
\end{itemize}

\subsection{Block C: Weil/CvS Numerics}
\label{sec:v32-block-c}

\begin{itemize}
  \item \textbf{RH-V4R foreign-operator run (terminal, independently
    verified).} One authorised run of a foreign operator returned code~$0$
    with checkpoints at $N = 48/56/64$. Its terminal result is
    \texttt{C13\_\allowbreak FIXED\_\allowbreak LADDER\_\allowbreak
    COMPLETE\_\allowbreak TAIL\_\allowbreak GATES\_\allowbreak
    NOT\_\allowbreak REACHED}: the Rayleigh tail passes, the fit tail fails
    ($0.1273\ldots > 0.0005$). Verification was independent on two levels
    --- the result verifier discarded $10/10$ mutations, the
    terminal-envelope verifier $17/17$, including a checkpoint byte
    mutation. The route reading is that this line has been measured, not
    confirmed; the open item \texttt{CVS\_\allowbreak N\_\allowbreak
    RESOLUTION\_\allowbreak UNRESOLVED} stands unchanged. Claim boundary:
    the failed fit tail is a gate result rather than a refutation of the
    underlying route, and no retry is admissible without a new contract.

  \item \textbf{K1 scalar audit, transitive source audit, and fail-closed
    full-replay input contract.} K1 remains open at the level of causes.
    The scalar audit ($256$-bit Arb) gives a fit-error quotient of
    $3.9533\ldots$, while the Rayleigh side spans only
    $1.4489\times 10^{-8}$. The transitive source audit shows that the
    wrapper does execute the V4 kernel but leaves a provenance gap, since
    path and SHA are missing from the manifests --- a historical defect
    that does not invalidate the run automatically. A fail-closed
    full-replay input contract now specifies exactly fourteen
    proof-carrying categories for $N = 40,\dots,64$, and \emph{none of
    these fourteen categories is currently present}; in the sense of
    \Cref{rem:v32-counts} the fourteen are the missing items, not passed
    checks. Claim boundary: the unit prices the resumption of K1 exactly,
    but it neither resumes it nor upgrades any claim.

  \item \textbf{Matrix production run D-20260813-003 (terminally verified,
    candidate missing).} A singly authorised Arb matrix production run
    ($c = 13$, $N = 48/56/64$, $T = 200$, $3225.75$~s) returned code~$0$
    and passed verification. The verifier reconstructed the
    $49\times 49$, $57\times 57$ and $65\times 65$ matrices, confirmed
    $10/10$ prime components and discarded $12/12$ mutations; the package
    is byte-identical (SHA \texttt{78ba72b5\ldots aab0e7}). The terminal
    status is \texttt{RIGOROUS\_\allowbreak MATRIX\_\allowbreak
    PRIMITIVES\_\allowbreak READY\_\allowbreak CANDIDATE\_\allowbreak
    MISSING}, recorded with \texttt{candidate\_\allowbreak
    verified\_\allowbreak here=false} and \texttt{claim\_\allowbreak
    upgrade=false}. What the run delivers is rigorous matrix primitives;
    what it does not deliver is the candidate --- candidate realisation,
    angle control, and the sector condition
    $\mu_{1,g^+} < \min(\mu_{2,g^-},\, \mu_{1,u^-})$ are separate gates
    that were not executed here. Claim boundary: primitives ready is not
    candidate verified, and the run explicitly upgrades no claim.
\end{itemize}

\subsection{Block D: Diagnostics and Hygiene}
\label{sec:v32-block-d}

\begin{itemize}
  \item \textbf{CCM--Groskin metadata package.} Twenty-one normalised
    cells of the metadata package were audited. The bound sources record no
    producer or verifier quotas for this unit, and none are supplied here.
    The package is explicitly \texttt{DIAGNOSTIC\_\allowbreak
    ONLY\_\allowbreak NOT\_\allowbreak VALIDATED}: it carries diagnostic
    value but no evidence level, and its negative blocks are warning
    signals rather than demonstrated spectral pollution. Open items include
    the $c = 100$ runner binding, the raw spectra, the smallest-positive
    branch selection, the classification, the spectral-pollution test, the
    operator realisation, the limit topology, and the $c/N/T$ transition.
    Claim boundary: nothing in this package supports or refutes a spectral
    statement, and reading its negative blocks as established pollution
    would over-read the material.

  \item \textbf{Residue/determinant hygiene gate (three-axis label
    audit).} A three-axis label audit covering $13/13$ claim lines, $7/7$
    active files and $21/21$ axes, with $14/14$ independent mutations
    discarded, established a bookkeeping rule that has kill effect. The
    rule keeps three channels strictly separate: the numerical
    eigenresidue, the meromorphic zero residue, and the determinant channel
    of \Cref{sec:det-channel}. New spectral files must continue this
    contract rather than restate it. Its operative consequence is the
    distinction that a technical PASS is not an analytical channel PASS ---
    a label conflating the three is void under the gate regardless of the
    numbers behind it. Claim boundary: the gate is a hygiene instrument, so
    it decides the admissibility of labels and not the truth of any
    spectral statement.
\end{itemize}

\subsection{Active Fronts (Status Lines Only)}
\label{sec:v32-fronts}

The following fronts are open at the time of this update. Each is given one
status line and no further detail; the working registers carry the
substance.

\begin{itemize}
  \item \textbf{Congruence reduction / residual gate G-A/G-B.} The
    structure theorem is available and the gate is defined but not
    executed, and a new contract with directed arithmetic is required.
  \item \textbf{21-node production run.} None of the twenty-one packages
    exists, and a separate production contract and authorisation are
    outstanding (the design uses $21$ collocation nodes at order $200$, not
    $200$ nodes).
  \item \textbf{N1/$\Phi_T$ front.} A separate front with its own register;
    not summarized here.
  \item \textbf{KLM branch maps.} Advanced as far as the normalised
    residual finding above, with no autonomously executable step available
    without new mathematical construction input.
\end{itemize}
