...
\]
für $\lambda \geq e^{13} = 442{,}413$, wobei $C < 2$ eine effektive Konstante ist, die die Verhältnisstruktur der Resolventen-Energien absorbiert; $C$ liegt numerisch im Bereich $[1{,}0, 1{,}6]$ auf dem getesteten Bereich, wurde aber nicht als rigorose geschlossene Konstante abgeleitet (siehe \Cref{rem:C-constant-caveat-de}). Da $0{,}005 < 0{,}0109/2$, erhalten wir:
\[
...
\begin{remark}[Status der Konstante $C$ im PNT-Transfer]
\label{rem:C-constant-caveat-de}
Die Schranke $C < 2$ oben wird heuristisch aus der Produktregel der relativen Störung $\Delta B/B$ in jedem Resolventen-Energie-Verhältnis abgeleitet; eine vollständig rigorose Herleitung müsste den Worst-Case der Kreuz-Terme zwischen Prim-Beitragsfehlern und Nenner-Lücken-Fehlern gleichmäßig in $j$ verfolgen. Numerische Auswertung auf $\lambda \in [500, 10^6]$ liefert effektive Werte $C_{\mathrm{eff}} \in [1{,}0,\, 1{,}6]$, komfortabel unterhalb der angerufenen Schranke. Eine rigorose geschlossene Schranke $C \leq C^\ast$ mit explizitem zulässigem $C^\ast$ bleibt noch zu etablieren; die Regime-2-Schwelle ist daher \emph{bedingt auf} $C < 2$. Der Sicherheitsspielraum $0{,}005$ erlaubt $C$ bis etwa $2{,}2$, bevor er aufgebraucht ist.
\end{remark}
