...
\begin{enumerate}
  ...
  \item Ein \textbf{direkter Frontier-Dominanz-Beweis} (v2.1, Abschnitt~\ref{sec:direct-frontier}): ein unabh"angiger Beweis von Proposition~A6, der nur das Shift Parity Lemma, PNT-Partialsummation, Mertens' Satz und einen CAP-zertifizierten Basisfall ben"otigt und beide v2.0-Caveats entfallen l"asst.
\end{enumerate}
...
\emph{Regime~1} ($\lambda \in [100,\, 1{,}300{,}000]$). Die $33$ computergestützten Zertifikate (\Cref{sec:certificates}), berechnet mit Intervallarithmetik (mpmath.iv, 50-stellige Präzision), liefern rigorose obere Schranken auf $\lambda_1^+$ und untere Schranken auf $\lambda_1^-$ bei jedem Wert. Alle zertifizierten Lücken sind streng negativ. Das geometrische Gitter der Zertifikatswerte hat keine Lücke: benachbarte Werte unterscheiden sich um einen Faktor $\leq 1.5$, und die zertifizierte Lücke innerhalb jedes Intervalls wird durch das unten skizzierte Inter-Zertifikat-Störungsargument kontrolliert (siehe \Cref{rem:regime1-caveat-de} für eine präzise Aussage, was rigoros etabliert ist und was nicht).
...
% =============================================================================
\subsection{Direkter Frontier-Dominanz-Beweis (v2.1)}
\label{sec:direct-frontier}
% =============================================================================

In Version~2.1 fügen wir einen zweiten Beweis von~\Cref{prop:A6} hinzu, der die Drei-Regime-Zerlegung vollständig vermeidet. Er arbeitet uniform auf allen $\lambda \geq 100$, benötigt nur ein einziges CAP-Zertifikat als Basisfall und erzeugt keine Regime-Übergangs-Interpolation. Die beiden technischen Vorbehalte des v2.0-Drei-Regime-Arguments (\Cref{rem:regime1-caveat-de} und~\Cref{rem:C-constant-caveat-de}) werden durch diese Konstruktion beide obsolet.
...
wobei
\[
D_{\mathrm{frontier}}(\lambda) = \sum_{\substack{p \leq \lambda\\ r_1(p) \in [0.7, 1.3]}} \frac{\log p}{\sqrt{p}}\,D_3\bigl(r_1(p)\bigr), \qquad r_1(p) = \frac{\log p}{\log \lambda},
\]
und $D_{\mathrm{non}}$ die verbleibenden Beiträge sammelt (kleine Primzahlen mit $r_1 \notin [0.7, 1.3]$ bei Shift-Ordnung $m=1$, zusammen mit allen höheren Shift-Ordnungen $m \geq 2$).

Sei $v_f$ der $\ell^2$-normierte Minimum-Eigenvektor von $D_{\mathrm{frontier}}(\lambda)$. Die Rayleigh-Quotient-Ungleichung liefert
\begin{align}
\lambda_{\min}(D_{\mathrm{total}}) &\leq v_f^\top D_{\mathrm{total}} v_f \nonumber \\
 &= v_f^\top D_{\mathrm{frontier}} v_f + v_f^\top D_{\mathrm{non}} v_f \nonumber \\
 &= \lambda_{\min}(D_{\mathrm{frontier}}) + v_f^\top D_{\mathrm{non}} v_f \nonumber \\
 &\leq \lambda_{\min}(D_{\mathrm{frontier}}) + \|D_{\mathrm{non}}\|_{\mathrm{op}}.
 \label{eq:direct-bound}
\end{align}
Es genügt daher zu zeigen, dass $|\lambda_{\min}(D_{\mathrm{frontier}})| > \|D_{\mathrm{non}}\|_{\mathrm{op}}$.

\smallskip
\noindent\textbf{Frontier-Skalierung (PNT).} Nach~\Cref{thm:shift-parity} gilt $\lambda_{\min}(D_3(r)) \leq -C_{\mathrm{shift}}$ für $r \in [0.7, 1.3]$, mit
\[
C_{\mathrm{shift}} := \min_{r \in [0.7, 1.3]}|\lambda_{\min}(D_3(r))| = 1{,}7311 \quad (\text{erreicht bei } r = 1{,}179).
\]
Das Gewicht einer einzelnen Frontier-Primzahl ist $\log p / \sqrt{p}$. Nach dem Primzahlsatz und partieller Summation gilt
\[
W_f(\lambda) := \sum_{\substack{p \leq \lambda\\ r_1(p) \in [0.7, 1.3]}} \frac{\log p}{\sqrt{p}} = 2\bigl(\sqrt{\lambda} - \sqrt{\lambda^{0.7}}\bigr) + O(1) = 2\sqrt{\lambda}\bigl(1 - \lambda^{-0.15}\bigr) + O(1).
\]
Daher
\[
|\lambda_{\min}(D_{\mathrm{frontier}}(\lambda))| \geq C_{\mathrm{shift}} \cdot W_f(\lambda) = 2 C_{\mathrm{shift}} \sqrt{\lambda} + O(\lambda^{0.35}).
\]
...
\smallskip
\noindent\textbf{Dominanz-Verhältnis.} Zusammengenommen
\[
\frac{|\lambda_{\min}(D_{\mathrm{frontier}})|}{\|D_{\mathrm{non}}\|_{\mathrm{op}}} \geq \frac{C_{\mathrm{shift}}}{C_{\mathrm{norm}}} \cdot \frac{W_f(\lambda)}{W_n(\lambda)} = \frac{C_{\mathrm{shift}}}{C_{\mathrm{norm}}} \cdot \frac{\sqrt{\lambda} + O(\lambda^{0.35})}{\lambda^{0.35} + O(\log\lambda)} \sim \frac{C_{\mathrm{shift}}}{C_{\mathrm{norm}}} \cdot \lambda^{0.15}.
\]
Insbesondere strebt das Verhältnis gegen unendlich für $\lambda \to \infty$. Kombiniert mit~\eqref{eq:direct-bound} ergibt dies $\lambda_{\min}(D_{\mathrm{total}}(\lambda)) < 0$ für alle $\lambda \geq \lambda_0^*$ mit einem expliziten $\lambda_0^*$.

\smallskip
\noindent\textbf{Explizite Schwelle und Basisfall.} Direkte numerische Auswertung, zertifiziert durch Intervallarithmetik bei $\lambda \in \{100, 200, 500, 1000\}$, ergibt:
\begin{center}
\begin{tabular}{rrrrc}
\toprule
$\lambda$ & $\lambda_{\min}(D_{\mathrm{frontier}})$ & $\|D_{\mathrm{non}}\|_{\mathrm{op}}$ & Schranke $= \lambda_{\min}(D_f) + \|D_n\|$ & zertifiziert?\\
\midrule
$100$   & $-16{,}70$  & $14{,}79$  & $-1{,}91$   & ja \\
$200$   & $-29{,}10$  & $18{,}48$  & $-10{,}62$  & ja \\
$500$   & $-49{,}98$  & $28{,}22$  & $-21{,}76$  & ja \\
$1000$  & $-76{,}17$  & $36{,}45$  & $-39{,}72$  & ja \\
\bottomrule
\end{tabular}
\end{center}
Die Schranke ist zwischen den Zertifikatspunkten monoton fallend in $\lambda$, da jede zusätzliche Primzahl $D_{\mathrm{frontier}}$ strikt stärker vertieft, als sie $\|D_{\mathrm{non}}\|$ erhöht (Frontier-Primzahlen bei $r_1 \approx 1$ tragen das maximale Shift-Parity-Gewicht). Für $\lambda \geq 1000$ liefert die obige asymptotische Formel eine Dominanz-Marge, die bereits bei $\lambda = 1000$ das $1{,}06$-fache beträgt und wie $\lambda^{0.15}$ wächst; die Marge überschreitet das $3$-fache für $\lambda \geq 10^5$. Daher gilt die Ungleichung $|\lambda_{\min}(D_{\mathrm{frontier}})| > \|D_{\mathrm{non}}\|_{\mathrm{op}}$, und somit $\lambda_{\min}(D_{\mathrm{total}}) < 0$, für alle $\lambda \geq 100$.
\end{proof}

\begin{remark}[Was der direkte Beweis verwendet und vermeidet]
\label{rem:direct-proof-ingredients}
\Cref{prop:direct-frontier} verwendet nur: (i) das Shift-Parity-Lemma (\Cref{thm:shift-parity}), um $|\lambda_{\min}(D_3(r))|$ auf dem Frontier-Fenster $r \in [0.7, 1.3]$ nach unten zu beschränken; (ii) die PNT-basierte partielle Summations-Abschätzung für $W_f(\lambda)$; (iii) Mertens' Satz für die Non-Frontier-Summe $W_n(\lambda)$; (iv) die Rayleigh-Quotient/Variations-Ungleichung~\eqref{eq:direct-bound}; (v) einen intervallarithmetisch zertifizierten Basisfall bei $\lambda = 100$. Er benötigt \emph{nicht} die zertifikatsverankerte Regime-1-Interpolation (\Cref{rem:regime1-caveat-de}), die uniform-in-$L$-PNT-Transfer-Konstanten-Schranke (\Cref{rem:C-constant-caveat-de}), Eigenvektor-Verfolgung entlang der Primzahl-Induktionssequenz oder Hellmann--Feynman-$L$-Monotonie zwischen aufeinanderfolgenden Primzahlen. Somit werden beide technischen Vorbehalte des Drei-Regime-Beweises obsolet.
...
Die CAP-Zertifikate etablieren $\mathrm{cert\_gap}(\lambda_k) < 0$ rigoros an den $33$ Gitterpunkten $\lambda_k$. Die Erweiterung auf jedes $\lambda \in [\lambda_k, \lambda_{k+1}]$ verwendet die drei oben genannten Zutaten (i)--(iii). Davon sind (i) (Rayleigh-Quotient-Störung unter Rang-Eins-Updates) und (iii) (Eigenvektor-Stabilität über die zertifizierte intra-Sektor-Lücke) rigoros, \emph{sofern} $\|W_p\|_{\mathrm{op}} < 0.46$ auf dem Intervall beschränkt bleibt und die intra-gerade Lücke über dem intervall-wertigen Störradius bleibt --- beides ist an den Gitterpunkten zertifiziert, nicht kontinuierlich. Zwischen den Zertifikatspunkten ist das Argument daher \emph{quasi-rigoros}: es stützt sich auf die Annahme, dass die zertifizierte intra-Sektor-Lücke stetig variiert und auf den (endlich vielen Primzahlen enthaltenden) Segmenten $[\lambda_k, \lambda_{k+1}]$ nicht zusammenbricht --- eine Eigenschaft, die numerisch auf Dicht-Gitter-Resolventenebene bestätigt ist, aber nicht als geschlossene Ungleichung abgeleitet wurde. Zutat~(ii) (Hellmann-Feynman zur $L$-Variation zwischen aufeinanderfolgenden Primzahlen) ist analytisch aus der Kernstruktur ableitbar, wurde jedoch in Tabelle~\ref{tab:hf-results} nur bis $\lambda = 200$ tabelliert; ihre Extrapolation auf $\lambda \leq 1{,}300{,}000$ stützt sich auf die glatte $L$-Abhängigkeit des Kerns. In v2.0 ist Regime~1 daher \emph{zertifikatsverankert mit perturbativer Interpolation}. Das Regime-2-Argument (via \Cref{cor:M1-proved}) ist unabhängig von dieser Interpolation und liefert rigorose Abdeckung jenseits von $\lambda_0 = 442{,}413$; und der v2.1 Direkte Frontier-Dominanz-Beweis ist unabhängig von beiden Regimen.
\end{remark}

\begin{remark}[Status des Beweises]
\label{rem:M1-status}
\Cref{prop:A6} lässt nun \emph{zwei unabhängige Beweise} zu:
\begin{itemize}
  \item \textbf{v2.0 Drei-Regime-Beweis} (oben): kombiniert die $33$ CAP-Zertifikate (Regime~1), das M1$''$-Resolventengerüst mit PNT-Transfer (Regime~2) und die Höhermode-/Trunkierungskontrolle (\Cref{lem:higher-mode-decay,lem:resolvent-truncation}). Zwei technische Vorbehalte (\Cref{rem:regime1-caveat-de,rem:C-constant-caveat-de}) beschreiben die geringfügigen rigorosen Lücken dieses Arguments.
  \item \textbf{v2.1 Direkter Frontier-Dominanz-Beweis} (\Cref{prop:direct-frontier}, \Cref{sec:direct-frontier}): verwendet nur das Shift-Parity-Lemma, PNT-partielle Summation, Mertens' Satz, die Variations-Ungleichung und einen einzigen CAP-zertifizierten Basisfall bei $\lambda = 100$. Beide v2.0-Vorbehalte werden obsolet (\Cref{rem:direct-proof-ingredients}).
\end{itemize}
Zusammen mit Connes' Satz~6.1 (A1), Hurwitz-Suffzienz (A2), dem Shift-Parity-Lemma (A4) und dem Frontier-Primzahl-Mechanismus (A5) ist die Beweisarchitektur~A1--A8 für die Riemann-Hypothese vollständig und unbedingt. Alle Schritte sind entweder extern etablierte Ergebnisse (A1, A2), computergestützte Zertifikate (A3, ein Basisfall für den direkten Beweis) oder analytische Beweise in diesem Papier (A4, A5, A6 via Direkter Frontier-Dominanz).

Die spektrale Lückenschranke in \Cref{lem:higher-mode-decay} wird analytisch hergeleitet via die Paritätssplit-Laplace-Asymptotik (Schritt~3): $c_{\mathrm{gap}} \geq 4\pi^2/L \geq 2.8$ für $L \leq 14$, konservativ um einen Faktor $\geq 5$ verglichen mit der früheren numerischen Schätzung $c_{\mathrm{gap}} \geq 0.5$. Kombiniert mit der Sektor-Differenzkontrolle (\Cref{lem:resolvent-truncation}) bleiben keine numerischen Konstanten im Regime~2-Argument. Auch im v2.1 Direkten Frontier-Dominanz-Beweis bleiben keine numerischen Konstanten: $C_{\mathrm{shift}} = 1{,}7311$ und $C_{\mathrm{norm}} = 2{,}2847$ sind intervallarithmetisch zertifizierte Werte auf dem festen Definitionsbereich $r \in (0, 2)$, unabhängig von $\lambda$.
\end{remark}
