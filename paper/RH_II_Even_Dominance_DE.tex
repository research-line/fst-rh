...
\label{prop:A6}
Die vorliegenden Drei-Regime- und Direct-Frontier-Argumente etablieren die asymptotische Variationsaussage
\[
  \lambda_{\min}(W^+_\lambda-W^-_\lambda)=-\Theta(\sqrt{\lambda})
\]
und eine endliche Brücke bedingt auf den Odd-Sektor-Tail-Lower-Bound. Die Eigenwertordnungs-Aussage
$\lambda_1(\mathrm{QW}_\lambda^{\mathrm{even}})
<\lambda_1(\mathrm{QW}_\lambda^{\mathrm{odd}})$ bleibt bedingt auf die
Asymptotische Variationsgap-Vermutung bzw. auf eine validierte untere Schranke
für $\lambda_1^-(\lambda)$.
\end{proposition}
