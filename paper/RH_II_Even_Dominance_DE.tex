...
\begin{enumerate}
  ...
  \item \textbf{Computergestützte finite-range Diagnostik} (\Cref{sec:certificates}): Starke negative Gap-Diagnostik für $33$ Werte von $\lambda$ zwischen $100$ und $1{,}300{,}000$ mittels Intervallarithmetik; als theoremstarke gerade Dominanz bleibt sie bedingt auf die validierte untere Schranke im ungeraden Sektor.
...
\begin{itemize}
  ...
  \item \textbf{Computer-gestützte Diagnostik (Intervallarithmetik):} Für $33$ Werte von $\lambda$ von $100$ bis $1{,}300{,}000$ wurde eine negative finite-range Gap-Diagnostik mittels Intervallarithmetik (mpmath.iv, 50~Ziffern) für den geraden Block und float64-Tail-Extrapolation für den ungeraden Block dokumentiert. Das Gap wächst monoton wie $\sim\!\sqrt{\lambda}$, von $-2.25$ bei $\lambda = 100$ bis $-475.83$ bei $\lambda = 1{,}300{,}000$; theoremstark ist dies bedingt auf die Odd-Sektor-Lower-Bound-Validierung (siehe \Cref{sec:certificates}).
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{Finite-range diagnostics:} For $33$ values of $\lambda$ from $100$ to $1{,}300{,}000$, the interval-arithmetic diagnostics give a strong conditional bridge, pending the odd-sector lower-bound validation (see \Cref{sec:certificates}).
...
Für $\lambda = 100$ und $\lambda = 200$ haben wir eine computergestützte finite-range Diagnostik unter Verwendung von Intervallarithmetik (mpmath.iv mit 50-stelliger Genauigkeit) abgeschlossen. Im aktuellen v2.4-Status ist dies eine bedingte Zertifikats-Pipeline mit drei Komponenten:
...
Die vorangegangenen Abschnitte etablieren eine starke finite-range Gap-Diagnostik für endlich viele $\lambda$ ($33$ Werte bis $\lambda = 1{,}300{,}000$; numerisch für alle $\lambda \geq 55$). Um den bedingten Beweis für \emph{alle} großen $\lambda$ zu schließen, skizzieren wir eine funktional-analytische Strategie basierend auf gewichteter Kompaktheit, die den kumulativen algebraischen Schritt vollständig vermeiden könnte.
...
Vor der vorliegenden Arbeit war die Gerade-Dominanz-Eigenschaft eine \emph{unbewiesene Annahme} in all approaches: Connes \cite{Connes2026} nimmt dies an, und Connes--van~Suijlekom \cite{ConnesvS2025} nehmen dies an in their Carath\'eodory--Fej\'er truncation argument. Klassische Ansätze via Krein--Rutman (positive Kerne) oder Sturm--Liouville (Knotentheorie) sind nicht anwendbar, da $A_\lambda$ einen \emph{indefiniten} Kern hat. Die vorliegende Arbeit reduziert diese Annahme auf die Odd-Sektor-Lower-Bound-/AVG-Schließung in \Cref{prop:A6}.
...
\end{enumerate}
Insbesondere existiert unter denselben Tail-Bound- und Evaluator-Annahmen eine Folge $\lambda_n \to \infty$, entlang derer die Voraussetzungen von Connes' Theorem~6.1 verifiziert sind; nach Fakt~6.4 und dem Satz von Hurwitz wird der RH-Schluss dann auf diese Odd-Sektor-Lower-Bound-Schließung reduziert.
\end{theorem}
...
Das Kopplungsdifferential $\sigma_1^+ - \sigma_0^-$ in \eqref{eq:gap-decomp} misst, wie viel staerker sich der ungerade Eigenwert unterhalb seiner Hauptdiagonalen verschiebt als der gerade Eigenwert. Die Störungstheorie zweiter Ordnung bietet den korrekten analytischen Rahmen zur Kontrolle dieser Größe.
...
\noindent Alle $33$ getesteten Werte von $\lambda \in [100, 1\,300\,000]$ ergeben $\mathrm{cert\_gap} < 0$, wobei $|\mathrm{cert\_gap}|$ monoton als $\sim\!\sqrt{\lambda}$ wächst. Dies ist eine computergestützte finite-range Diagnostik über mehr als $4$ Größenordnungen in~$\lambda$, konsistent mit der bedingten Vorhersage von \Cref{thm:meta}.
...
\begin{itemize}
  \item \textbf{Unbedingt:} Shift-Parity-Lemma, Leading-Mode-Cancellation, NE-A sowie die endlichen NE-B-Rechnungen; außerdem negative finite-range Gap-Diagnostik auf $100\leq\lambda\leq1{,}300{,}000$ bedingt auf den Odd-Sektor-Tail-Lower-Bound.
...
\end{itemize}
Zusammen mit Connes' Satz~6.1 (A1) und Fact~6.4/Hurwitz-Suffizienz (A2) ist die aktuelle Beweisarchitektur daher eine \emph{conditional reduction}, kein unbedingter Beweis der Riemannschen Vermutung.
