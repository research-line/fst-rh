...
mit sättigendem $\kappa_\infty \in [1.6, 1.8]\times 10^{-3}$ wird
empirisch etabliert und liefert Theorem~B-1 (bedingt):
...
Fünfzehn ausgeschlossene spektrale und CCM-Wege werden katalogisiert,
...
\begin{itemize}
  \item \textbf{C2ca.1 Skalare säkulare Auslöschung:} algebraische
    ...
    Numerische Evidenz:
    $|\mu/\alpha|\approx 4\text{--}5\times 10^{-7}$ stabil über
    $\lambda = 3, 5, 7$.
...
$1+\kappa(\lambda)\delta^2$. Empirisch sättigt
$\kappa(\lambda)$ zu
$\kappa_\infty \in [1.6, 1.8]\times 10^{-3}$ über
$\lambda = 3,5,7,9,11,13,15,20$, mit
$\kappa(20)=1{,}527\times 10^{-3}$. Diese Sättigung ist konsistent mit der
Foundation-Lock-Konvergenz: für $\lambda\to\infty$ konvergiert
$\hat\xi_\lambda \to \Xi$ in der relevanten Topologie, und die residuale
Krümmung misst die Finite-$\lambda$-Korrektur.
...
B & $\kappa_\infty$ (residuale Krümmung) & $\in [1.6,1.8]\times 10^{-3}$ & empirisch (gesättigt) \\
...
6 ausgeschlossene spektrale Wege + 9 ausgeschlossene CCM-Wege & dokumentiert & \Cref{sec:excluded-spectral,sec:excluded-ccm} \\
