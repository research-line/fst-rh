...
\title{A Conditional Proof of the Riemann Hypothesis via Even Dominance of the Weil Quadratic Form}
\author{Lukas Geiger\\Independent Researcher, Bernau\\ORCID: 0009-0005-7296-1534}
\date{Draft v1.1 -- 3 May 2026}

\begin{document}
\maketitle

\begin{abstract}
This paper presents a conditional proof of the Riemann Hypothesis, extracted
from a three-part Landscape series. The Landscape papers document the full
research process, including failed routes, obstruction theorems, computational
certificates, and alternative strategies. The present paper keeps only the
proof-relevant chain.
Following Connes' reduction of the Riemann Hypothesis to an even-dominance
condition for the Weil quadratic form, we prove that the relevant ground state is
simple and even along a sequence $\lambda_n\to\infty$. The proof has four inputs:
(i) Connes--van Suijlekom's real-zero criterion for quadratic forms
(\cite[Theorem~6.1]{Connes2026}, together with the Hurwitz bridge of
\cite[Fact~6.4 / Section~6.4--6.6]{Connes2026}); (ii) the Shift Parity Lemma, a finite
trigonometric matrix theorem showing that every admissible prime shift has an
intrinsic even-sector advantage; (iii) interval-arithmetic certificates for the
finite range $100\leq\lambda\leq 1{,}300{,}000$; and (iv) a frontier-dominance
argument based on the Prime Number Theorem, Mertens-type estimates, and a common
Rayleigh-vector bound for primes $p$ with $\log p/\log\lambda$ near $1$. Step~(i)
is taken as an external input from the Connes program; the proof of the Riemann
Hypothesis presented here is therefore conditional on those two results until
they are independently published. Steps~(ii)--(iv) are self-contained and
analytical, with the finite range and the asymptotic threshold meeting exactly
at $1{,}300{,}000$. The excluded gauge, total-positivity, and universal-commutator
routes are not used in the proof and are referred to the Landscape series.
\end{abstract}

\noindent\textbf{Keywords:} Riemann Hypothesis, Weil quadratic form, even dominance, Connes program, Shift Parity Lemma, computer-assisted proof.
...
The proof presented here uses the following implication chain:
\[
\begin{gathered}
\text{Shift parity}+\text{frontier dominance}+\text{finite certificates}\\
\Longrightarrow\ \text{even dominance of }\QW_\lambda\\
\Longrightarrow\ \text{RH via Connes and Hurwitz}.
\end{gathered}
\]

\section{Connes' Reduction}

Let $L=\log\lambda$ and consider the Hilbert space $L^2([-L,L])$. In additive
coordinates $u=\log x$, Connes' Weil quadratic form is represented by a
self-adjoint operator $A_\lambda$ of the form
\begin{multline}
\label{eq:weil-form}
...
\end{multline}
where $T_u$ is the shift operator. The operator commutes with parity, hence it
splits into even and odd sectors:
\[
...
\begin{definition}[Even dominance]
\label{def:even-dominance}
We say that $\QW_\lambda$ has even dominance if
\[
  \lambda_1^+(\lambda)<\lambda_1^-(\lambda)
\]
and the even-sector minimum is simple and isolated.
\end{definition}
...
The proof begins with a finite algebraic fact. Let
\[
...
A negative certified gap implies even dominance.
\end{sloppypar}
...
Landscape certifier verifies
\(
  \lambda_{\min}(\mathrm{QW}_{\sin}^{(N)})\;>\;\lambda_{\min}(\mathrm{QW}_{\cos}^{(4)})
\)
for \emph{all} $N\leq 90$ at every tested $\lambda$, confirming that the gap is
genuine and not a truncation artifact.
\end{remark}

\begin{remark}[Truncation safety factor]
\label{rem:truncation-safety}
The combined Galerkin/tail truncation error of the Landscape certifier at $\lambda=100$
...
the safety factor exceeds $65\times$ and grows monotonically with $\lambda$ throughout
the certified range~\cite[Section~2.9 and certificate metadata]{GeigerLandscapeII}.
\end{remark}

\begin{proposition}[Certified finite bridge]
\label{prop:finite-certificates}
For every certified grid point $\lambda_k$ in the Landscape table,
\[
  \Delta(\lambda_k)<0.
\]
Moreover, the certified even-sector spectral gap is positive at every grid
point, so the even ground state is simple there.
\end{proposition}

\begin{proof}
This is a computer-assisted proof input from the Landscape project
\cite{GeigerLandscapeII}. The relevant certificates are stored in the Landscape
...
\end{array}
\]
All intervals have negative certified gap. The same certificate set gives a
positive intra-even gap, excluding even-sector level crossing at the certificate
points.
\end{proof}

\begin{remark}[Certificate structure]
\label{rem:cert-structure}
Each certificate for a given $\lambda_k$ consists of:
...
(d)~a rigorous lower bound $\underline\lambda_1^-$ for the minimum odd eigenvalue;
(e)~the certified gap $\overline\lambda_1^+-\underline\lambda_1^-<0$.
For instance, at $\lambda=100$: the $4\times 4$ even matrix has minimum eigenvalue
in $[-11.08,-11.06]$, the odd minimum lies above $-8.82$, giving certified gap
$<-2.24$. All certificates, including the interval-arithmetic evaluation scripts,
are archived in~\cite{GeigerLandscapeII}.
\end{remark}

\section{Frontier Dominance}

The asymptotic proof uses the fact that primes near the moving frontier
...
\begin{proposition}[Asymptotic frontier dominance]
...
the certified range $100\leq\lambda\leq 1{,}300{,}000$ of
\Cref{prop:finite-certificates} therefore meet without gap.
\end{proof}
...
\noindent Combining (i)--(iii) with the certified range $[100,\,1{,}300{,}000]$
gives $\Lambda_*\leq 175{,}000$.
The verification script \texttt{verify\_lambda\_star.py} (float64, safety margins
$\gg 10^{-10}$) is archived in the companion repository at
\url{https://github.com/research-line/rh-even-dominance}.
\end{remark}

\section{Tail Control and Lifting to the Weil Form}
\label{sec:tail}

\Cref{prop:frontier-dominance} establishes $\lambda_{\min}(D_{\mathrm{tot}}(\lambda))<0$
in the three-mode difference block. The full Weil operator $A_\lambda$ acts on
$L^2([-L,L])$ with infinitely many Fourier modes and includes the archimedean
integral. We show that the contributions beyond the low-mode prime block
preserve the sign of the even--odd gap.

\begin{remark}[Archimedean contribution]
\label{rem:archimedean}
The archimedean kernel $e^{u/2}/(2\sinh u)$ in~\eqref{eq:weil-form} is integrable
...
consistent with the Weil form~\eqref{eq:weil-form}.
\end{definition}
...
\]
At every certified grid point this analytical bound is dominated by the
interval-certified Landscape safety factor of \Cref{rem:truncation-safety}
($\geq 34\times$ at $\lambda=100$, $\geq 65\times$ for $\lambda\geq 200$),
which is the rigorous statement used in the certified gap.
\end{lemma}
...
The displayed bound $16\sqrt\lambda/N_0^2$ follows. The certified gap uses the
sharper interval-arithmetic enclosure of \cite[Section~2.9]{GeigerLandscapeII}
with the Cauchy tail bound described in \Cref{rem:truncation-safety}.
\end{proof}

\section{Main Theorem}

\begin{theorem}[Even dominance for a divergent sequence]
\label{thm:even-sequence}
There exists a sequence $\lambda_n\to\infty$ such that $\QW_{\lambda_n}$ has a
simple even ground state for every $n$.
\end{theorem}

\begin{proof}
For $100\leq\lambda\leq1{,}300{,}000$, the certified grid of
\Cref{prop:finite-certificates} gives even dominance and even-sector simplicity
at the listed values; these certificates evaluate the full Galerkin operator
(including archimedean and higher-mode contributions).

For $\lambda$ beyond the asymptotic threshold,
\Cref{prop:frontier-dominance} gives a three-mode gap
$\Delta_{\mathrm{low}}=-\Theta(\sqrt\lambda)$. The higher-mode correction satisfies
$|\Delta_{\mathrm{high}}|=O(\sqrt\lambda/L)=o(|\Delta_{\mathrm{low}}|)$ by
\Cref{lem:higher-mode}(iii), the archimedean contribution is $O(1)$ by
\Cref{rem:archimedean}, and the Galerkin truncation error is
$\leq 16\sqrt\lambda/N_0^2$ by \Cref{lem:galerkin} -- a fraction $4/N_0^2$ of the
leading $\sqrt\lambda$-gap, hence $o(|\Delta_{\mathrm{low}}|)$ for $N_0\geq 40$.
Therefore the full even--odd gap is negative for all
sufficiently large $\lambda$, with the asymptotic threshold meeting the
certified range exactly at $\Lambda_*\leq 175{,}000$
(see \Cref{rem:lambda-star-improved}; the conservative upper bound
$\Lambda_*\leq 1{,}300{,}000$ of \Cref{prop:frontier-dominance} is also valid).
Taking any divergent sequence of certified or asymptotic values gives even
dominance.

Simplicity of the even ground state follows from the intra-even spectral gap
$\lambda_2^+-\lambda_1^+=\Theta(\sqrt\lambda)$: at the certificate points this is
directly verified (\Cref{prop:finite-certificates}); for large $\lambda$ it follows
from the Galerkin diagonal asymptotics, since the leading even mode ($n=1$) has
$(W_{\mathrm{prime}})_{11}\sim -\sqrt\lambda$ while the next mode ($n=0$) has a
different Laplace endpoint value.
\end{proof}

\begin{theorem}[Riemann Hypothesis, conditional on the Connes program]
\label{thm:rh}
Conditional on \cite[Theorem~6.1 and the Hurwitz bridge of Section~6.4--6.6]{Connes2026}
and \cite{ConnesvS2025}, all non-trivial zeros of the Riemann zeta
function~\cite{Riemann1859} lie on the line $\Re(s)=1/2$.
\end{theorem}

\begin{proof}
Apply \Cref{cor:sequence-suffices} to the sequence supplied by
\Cref{thm:even-sequence}. The Fourier transforms associated with the
even-dominant Weil forms have only real zeros by \Cref{thm:connes}. Hurwitz
transfer gives the real-zero property for the limiting $\Xi$-function. Therefore
the corresponding non-trivial zeros of $\zeta(s)$ lie on the critical line.
\end{proof}
...
\end{itemize}
see~\cite[\texttt{scripts/}]{GeigerLandscapeII}. A dedicated companion repository
for the present extraction is mirrored at
\url{https://github.com/research-line/rh-even-dominance}.
