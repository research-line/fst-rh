...
for the sine-block lower bound; the 14 May 2026 v1.4 revision documents
this explicitly in
...
(script \path{_proof-notes/verify_connes_vs_form.py}).  The
...
\]
denote the $L^2([-L,L])$-normalised even and odd basis functions. For a normalised shift $r=s/L$ define
...
proving that the gap is genuine and not a truncation artefact.
\end{remark}
...
\path{verify_connes_vs_form.py}, see
...
For the even basis function $\varphi_n$ and normalised shift $u=\delta/L\in[0,2]$,
...
shifts cancel under symmetrisation. Numerical verification
...
Fourier orthogonality (the sine-cross-terms cancel under symmetrisation),
...
\sup_{x\in\mathbb{R}}|\hat\xi_\lambda(x)|$, the global
strip-amplification ratio used in the scripts and in \S{}E.18) and
...
constitute a proof of~(ii-a).  The earlier real-axis
polynomial-weight formula attached to this table was a transcription
error; the table values are strip-amplification diagnostics.  The
bias comparison with $A_\Xi$ used in the proof notes is a
project-internal diagnostic and falsification test, not a
theorem-level equivalence to RH and not a substitute for proving
the a-priori strip bound.  Earlier measurements reported in
...
\begin{itemize}[leftmargin=*]
  ...
  \item \emph{Finite Sylvester inertia $\nu_-=0$.} Interval-arithmetic
    $\mathrm{LDL}^\top$ inertia ($\lambda=100$, $N\le 40$) certifies a
    finite-section inertia statement for the computed truncated matrix. This
    improves the finite-stage diagnostic beyond the earlier heuristic tail
    extrapolation, but it does not yet certify the infinite operator or the full
    finite range; a tail, Feshbach, or Schur coercivity step remains open.
\end{itemize}
...
\paragraph{17 June 2026 far-ring ledger.}
A later diagnostic splits the far contribution into dyadic rings relative to
the collar edge and records signed load, $L^1$ mass, imbalance, first-ring
share, and scaled mode ratios. This sharpens the open slot from a generic
signed far-tail cancellation claim to
\texttt{QA2\_FAR\_RING\_IMBALANCE\_BOUND}. It is diagnostic only and gives no
DZT2, MS2, even-dominance, or RH claim upgrade.
...
All explicit numerical constants cited in the argument or diagnostic audit, with
their source, role, and verification mechanism. Constants flagged ``IA'' are
interval-arithmetic certified (\texttt{mpmath.iv}, $50$-digit precision);
``legacy CAP diagnostic'' refers to the $33$ Landscape computer-assisted
certificate computations as numerical diagnostics, not as a theorem-level CAP
closure; ``PNT/Mertens'' are explicit asymptotic remainders.
...
$N_{\sin}$ & $40$--$90$ & Odd-block Galerkin truncation (\Cref{rem:galerkin-asymmetry}) & Diagnostic Cauchy-tail estimate\\
...
$|\mathrm{cert\_gap}|$ at $\lambda{=}100$ & $\geq 2.25$ & First legacy CAP diagnostic (\Cref{prop:finite-certificates}) & Legacy CAP diagnostic\\
$|\mathrm{cert\_gap}|$ at $\lambda{=}1.3{\cdot}10^6$ & $\geq 475.83$ & Last legacy CAP diagnostic & Legacy CAP diagnostic\\
Safety factor at $\lambda{=}100$ & $\geq 34\times$ & $|\mathrm{cert\_gap}|$ vs.\ Galerkin/tail error (\Cref{rem:truncation-safety}) & IA + diagnostic Cauchy-tail estimate\\
Safety factor at $\lambda\geq 200$ & $\geq 65\times$ & Same, monotone in $\lambda$ & IA + diagnostic Cauchy-tail estimate\\
...
\begin{itemize}[leftmargin=*,itemsep=1pt,topsep=2pt,parsep=0pt]
  ...
  \item \textit{Formatting:} Preparation and verification of citations,
        bibliographies, and \LaTeX{} typesetting.
