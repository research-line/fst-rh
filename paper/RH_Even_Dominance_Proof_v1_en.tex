...
First, for the finite range $100\leq\lambda\leq 1{,}300{,}000$, even dominance
$\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$ is rigorously certified by
interval-arithmetic Galerkin bounds that produce a separate upper bound for
$\lambda_1^+$ and a separate lower bound for $\lambda_1^-$ at each certificate
point. Second, for the asymptotic regime $\lambda\to\infty$, we establish via
...
the Hurwitz limit-passage of \cite[Sec.~6.4--6.6]{Connes2026}, which is
described there as proof strategy, not theorem), and (ii) the
...
\[
\begin{gathered}
\text{Shift parity}+\text{IA certificates for }100\leq\lambda\leq 1{,}300{,}000\\
\Longrightarrow\ \text{Even Dominance on the certified range (\Cref{thm:even-finite})},
\end{gathered}
\]
together with
\[
...
\end{gathered}
\]
The first line is unconditional. The second line introduces two conditional
inputs: the Connes program (external) and the Asymptotic Variational Gap
Conjecture (\Cref{conj:avg}, internal to this approach;
...
\paragraph{Migration of downstream sections (v1.5).}  The above
...
$\sqrt\lambda$ scale.  Even dominance therefore holds asymptotically
with margin $O(\sqrt\lambda/\log\lambda)$ rather than $O(\sqrt\lambda)$
as the legacy direct-frontier argument suggested.  The Legacy
...
archimedean integrand.  The estimates below carry over with the
modification that the relevant overlap functions are
$\sin(2\pi m y/L) - \sin(2\pi n y/L)$ in place of $D_3(r)$ and the
Schur-complement bound uses the off-block norm of the
Connes--van Suijlekom matrix.  Migration of the precise inequalities
...
\begin{itemize}[leftmargin=*]
  \item \emph{External input from Connes 2026 §6.5
...
$\Lambda_n\to\infty$, together with the proven Connes--van Suijlekom
criterion, gives RH via Fact~6.4 of Connes~2026.  The companion
...
\begin{theorem}[Even dominance on the certified range]
\label{thm:even-finite}
For every $\lambda$ in the certified grid of \Cref{prop:finite-certificates}
(33 values spanning $100\leq\lambda\leq 1{,}300{,}000$),
$\QW_\lambda$ has a simple even ground state, i.e.\ $\lambda_1^+(\lambda)<\lambda_1^-(\lambda)$
and the even minimum is isolated.
\end{theorem}

\begin{proof}
The interval-arithmetic Galerkin certificates of \Cref{prop:finite-certificates}
produce, at each $\lambda$ in the grid, a rigorous upper bound
$\overline{\lambda_1^+}(\lambda)$ and a rigorous lower bound
$\underline{\lambda_1^-}(\lambda)$ from the even and odd Galerkin blocks
respectively, together with Cauchy-type tail bounds. The certified gap
...
the certified range. Simplicity follows from the certified intra-even gap
...
On the certified range this gap is filled by the direct computation of
$\underline{\lambda_1^-}$ used in \Cref{thm:even-finite}; asymptotically a
...
\begin{proof}
By \Cref{thm:even-finite}, even dominance holds at every $\lambda$ in the
certified grid; by \Cref{conj:avg} it holds for every $\lambda\geq\Lambda^\dagger$.
Either provides a divergent sequence $\lambda_n\to\infty$ of even-dominant
parameters. Applying \Cref{cor:sequence-suffices} to this sequence: the Fourier
...
\paragraph{Step 1 (Diagonal accounting).} Compute the explicit diagonal entries
of $W^\pm_{\mathrm{prime}}$ on the three-mode block, separating leading
...
consistent with the numerical observation that the certified gap is roughly
...
asymptotically nor on the certified range, unless one of the
...
certified gap are unaffected by this concern and remain valid as
...
\texttt{\_proof-notes/K0\_STEP1\_FINDINGS.md} and
\texttt{\_proof-notes/K0\_CAP\_AUDIT.md} provide the full analytical
...
computation (script \texttt{\_proof-notes/verify\_connes\_vs\_form.py})
...
\texttt{verify\_connes\_vs\_form.py}, derivation in
\texttt{K0\_FRONTIER\_SN\_ASYMPTOTIC.md}).  The structural foundation
...
\texttt{\_proof-notes/K0\_LEHMANN\_ANALYSIS.md}, with the conclusion:
