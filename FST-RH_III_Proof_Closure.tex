...
\subsection{Status: What is and is not proved}
\label{sec:status-honest}

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{l|l|l}
\textbf{Statement} & \textbf{Status} & \textbf{Method} \\\hline
$S^+(\delta) < S^-(\delta)$ for $\delta \in (0,L)$ & \textbf{Proved} & Exact identity (\Cref{cor:diagonal-parity}) \\
$W_{11}^+ < W_{00}^-$ for large $\lambda$ & \textbf{Proved} & Shift Parity + PNT (\Cref{lem:L1}) \\
$\|c_{\mathrm{high}}\|^2 \to 0$ & \textbf{Proved} & Schur complement (\Cref{prop:mode-concentration}) \\
Even dominance at $\lambda = 100, 200$ & \textbf{Proved} & Interval arithmetic (\Cref{sec:cap}) \\[4pt]
Even dominance for all $\lambda \geq \lambda_0$ & \textbf{Open} & Coupling asymmetry unresolved \\
$\Delta(\lambda) \to -\infty$ & \textbf{Conjectured} & Numerical: $|\Delta| \sim (\log\lambda)^{3.5}$
\end{tabular}
\end{center}

\noindent The gap between ``$W_{11}^+ < W_{00}^-$'' (proved) and ``$\lambda_1^+ < \lambda_1^-$'' (desired) is precisely the coupling asymmetry described in \Cref{rem:coupling-revisited}. Closing this gap requires either:
\begin{enumerate}[label=(\alph*)]
  \item An analytical bound showing $\|B^-\|^2 - \|B^+\|^2 = o(\sqrt{\lambda})$ (coupling asymmetry is sublinear), or
  \item Extension of the computer-assisted proof to a dense grid of $\lambda$-values (e.g., $\lambda = 100, 200, 300, \ldots, 10000$), combined with a continuity/monotonicity argument between grid points, or
  \item A fundamentally different approach to the lower bound on $\lambda_1^-$.
\end{enumerate}

\noindent Option~(b) is the most realistic near-term strategy: Connes' Hurwitz argument requires even dominance only along a \emph{sequence} $\lambda_n \to \infty$, so a dense computer-assisted grid would suffice. Option~(a) would give the strongest result but requires a deeper understanding of the parity structure of the off-diagonal blocks.

\begin{remark}[What the diagonal analysis does achieve]
Even though the coupling prevents a clean $\Delta \sim W_{11}^+ - W_{00}^-$ reduction, the Shift Parity identity (\Cref{cor:diagonal-parity}) and PNT estimate (\Cref{lem:L1}) provide the \emph{only known structural mechanism} for even dominance. They explain \emph{why} the phenomenon occurs (the cosine mode has a zero where prime shifts concentrate) and predict the correct scaling. Any future proof must ultimately capture this mechanism.
\end{remark}
