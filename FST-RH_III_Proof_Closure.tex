...
The certifier program (\texttt{certifier\_production.py}) implements the certified-gap computation for a geometric grid of $\lambda$-values. The even block uses $k = 4$ modes with interval arithmetic (\texttt{mpmath.iv}, 50-digit precision); the odd block uses $N_0 = 40$--$90$ modes in float64 with a Cauchy tail bound.
...
\begin{enumerate}[label=\textup{(\alph*)}]
  ...
  \end{align*}
  The first follows from the closed form $S^+_{\mathrm{sym}}(1,0;u) = \sqrt{2}\sin(\pi u)/\pi$; the second from $S^+_{\mathrm{sym}}(1,2;u) = \frac{2}{3\pi}[2\sin(2\pi u) - \sin(\pi u)]$. Therefore $c = \sqrt{2} + 2$.
  \item For $j \geq 2$: $\Delta_j(u) = O(u)$ individually, i.e., $\Delta_j(u) \cdot L$ converges as $L \to \infty$ at each fixed $\delta = Lu$.
\end{enumerate}
\end{lemma}

\noindent Combined with the resolvent truncation (only finitely many modes $j$ contribute to $E$), this structure implies that the effective overlap difference in the prime sum is $O(u) = O(\log p / L)$ for each prime $p$. After summing over primes with weights $a_p = (\log p)/\sqrt{p}$ and applying the PNT:
\[
  \sum_{p \leq \lambda} a_p \cdot O\!\bigl(\tfrac{\log p}{L}\bigr) = O\!\Bigl(\frac{1}{L}\sum_{p \leq \lambda}\frac{(\log p)^2}{\sqrt{p}}\Bigr) = O\!\Bigl(\frac{\sqrt{\lambda}\log\lambda}{L}\Bigr) = O(\sqrt{\lambda}).
\]
After squaring and dividing by $\mathrm{gap}_j \sim c_j\sqrt{\lambda}$, this gives $E_{\sin} - E_{\cos} = O(\sqrt{\lambda}/L^2)$, and therefore
\[
  \frac{E_{\sin} - E_{\cos}}{D(\lambda)} = O(1/L) \;\xrightarrow{L \to \infty}\; 0.
\]
