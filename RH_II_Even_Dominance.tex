...
\emph{Step~3 (Gap lower bound with parity split).} The auto-overlap profile is $h_n(u) = \tfrac{1}{2}(1-u/2)\cos(n\pi u) - \sin(n\pi u)/(4n\pi)$. At the Laplace endpoint $u = 1$: $h_n(1) = (-1)^n/4$. The prime-weighted diagonal is dominated by this endpoint via the Laplace principle:
\[
...
\emph{Step~4 (Parity-matched coupling suppression).} For odd $j \geq 3$, the \emph{cross}-overlap $S^+(1,j;\delta,L)$ also vanishes at the Laplace endpoint: $S^+(1,j;L,L) = (-1)^j\delta_{1j}/2 = 0$ for $j \neq 1$ (by orthogonality on $[0,L]$). The coupling $B_{1j}$ therefore comes from the subleading Laplace term, giving $|B_{1j}| = O(j\sqrt{\lambda}/L)$ instead of $O(\sqrt{\lambda})$. The ratio:
\[
...
\begin{proof}
By \Cref{prop:euler-maclaurin}, $\rho^{\mathrm{EM}}(L) < 0$ for $L \geq 13$ (i.e., $\lambda \geq 442{,}413$), certified by interval arithmetic. By \Cref{lem:pnt-transfer}, $|\rho(\lambda) - \rho^{\mathrm{EM}}(\lambda)| = O(L\,e^{-c\sqrt{L}}) \to 0$. Choose $L_1 \geq 13$ such that $|\rho(\lambda) - \rho^{\mathrm{EM}}(\lambda)| < |\rho^{\mathrm{EM}}(L_1)|/2$ for all $\lambda \geq e^{L_1}$. Then for $\lambda_0 \coloneqq e^{L_1}$:
\[
...
Between adjacent certificate values, the certified gap cannot cross zero by a structural argument: (i)~by the Shift Parity Lemma (\Cref{thm:shift-parity}), each new prime $p$ entering the sum at $\lambda = p$ satisfies $\lambda_1(W_p^+) < \lambda_1(W_p^-)$, so new primes \emph{deepen} the even--odd gap; (ii)~by the Hellmann--Feynman analysis, $d(\mathrm{gap})/dL > 0$ for $\lambda \geq 80$, so the $L$-variation also deepens the gap; (iii)~the intra-even spectral gap ($\lambda_2^+ - \lambda_1^+ \geq 8.69$ at $\lambda = 100$, \Cref{lem:simplicity}) exceeds the operator norm of any single prime perturbation ($\|W_p\|_{\mathrm{op}} < 0.46$ for $p \geq 100$) by a factor $\geq 18$, ensuring eigenvector stability under each discrete prime addition.
